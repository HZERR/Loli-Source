...
public class PNGDecoder {
    ...
    private void copyRGBAtoBGRA(ByteBuffer buffer, byte[] curLine) {
        int n2 = curLine.length;
        for (int i2 = 1; i2 < n2; i2 += 4) {
            buffer.put(curLine[i2 + 2]).put(curLine[i2 + 1]).put(curLine[i2]).put(curLine[i2 + 3]);
        }
    }
