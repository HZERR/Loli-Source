...
public class ScriptMapper {
    ...
    public static synchronized int getScript(int n2) {
        if (n2 >= data[cache] && n2 < data[cache + 2]) {
            return data[cache + 1];
        }
        if (n2 < 0 || n2 >= CHAR_LIMIT) {
            return 103;
        }
        int n3 = 512;
        int n4 = 0;
        if (n2 >= data[dataExtra]) {
            n4 = dataExtra;
        }
        while (n3 > 2) {
            if (n2 < data[n4 + (n3 >>= 1)]) continue;
            n4 += n3;
        }
        cache = n4;
        return data[n4 + 1];
    ...
    public static boolean isComplexCharCode(int n2) {
        if (n2 < 768 || n2 > 8303) {
            return false;
        }
        if (n2 <= 879) {
