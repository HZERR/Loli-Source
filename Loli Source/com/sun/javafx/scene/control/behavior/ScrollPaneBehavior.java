...
extends BehaviorBase<ScrollPane> {
    ...
    public void contentDragged(double d2, double d3) {
        ScrollPane scrollPane = (ScrollPane)this.getControl();
        if (!scrollPane.isPannable()) {
            return;
        }
        if (d2 < 0.0 && scrollPane.getHvalue() != 0.0 || d2 > 0.0 && scrollPane.getHvalue() != scrollPane.getHmax()) {
            scrollPane.setHvalue(scrollPane.getHvalue() + d2);
        }
        if (d3 < 0.0 && scrollPane.getVvalue() != 0.0 || d3 > 0.0 && scrollPane.getVvalue() != scrollPane.getVmax()) {
            scrollPane.setVvalue(scrollPane.getVvalue() + d3);
        }
    }
