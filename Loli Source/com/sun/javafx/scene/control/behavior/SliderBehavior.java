...
extends BehaviorBase<Slider> {
    ...
    public void trackPress(MouseEvent mouseEvent, double d2) {
        Slider slider = (Slider)this.getControl();
        if (!slider.isFocused()) {
            slider.requestFocus();
        }
        if (slider.getOrientation().equals((Object)Orientation.HORIZONTAL)) {
            slider.adjustValue(d2 * (slider.getMax() - slider.getMin()) + slider.getMin());
        } else {
            slider.adjustValue((1.0 - d2) * (slider.getMax() - slider.getMin()) + slider.getMin());
        }
    }

    public void thumbPressed(MouseEvent mouseEvent, double d2) {
        Slider slider = (Slider)this.getControl();
        if (!slider.isFocused()) {
            slider.requestFocus();
        }
    ...
    public void thumbDragged(MouseEvent mouseEvent, double d2) {
        Slider slider = (Slider)this.getControl();
        slider.setValue(com.sun.javafx.util.Utils.clamp(slider.getMin(), d2 * (slider.getMax() - slider.getMin()) + slider.getMin(), slider.getMax()));
    }
