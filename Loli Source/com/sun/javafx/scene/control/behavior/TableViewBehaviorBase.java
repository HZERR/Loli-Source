...
extends BehaviorBase<C> {
    ...
    protected void callAction(String string) {
        boolean bl;
        boolean bl2 = bl = ((Node)this.getControl()).getEffectiveNodeOrientation() == NodeOrientation.RIGHT_TO_LEFT;
    ...
    public TableViewBehaviorBase(C c2, List<KeyBinding> list) {
        super(c2, list == null ? TABLE_VIEW_BINDINGS : list);
    }
    ...
    protected void clearSelectionOutsideRange(int n2, int n3, TableColumnBase<T, ?> tableColumnBase) {
        ...
        for (int i2 = 0; i2 < arrayList.size(); ++i2) {
            ...
            tableSelectionModel.clearSelection(n6, tableColumnBase);
        }
    ...
    protected void updateRowSelection(int n2) {
        ...
        int n3 = tableFocusModel.getFocusedIndex() + n2;
        TablePositionBase tablePositionBase = this.getAnchor();
        if (!this.hasAnchor()) {
            this.setAnchor(this.getFocusedCell());
        }
        if (tableSelectionModel.getSelectedIndices().size() > 1) {
            this.clearSelectionOutsideRange(tablePositionBase.getRow(), n3, null);
        }
        if (tablePositionBase.getRow() > n3) {
            tableSelectionModel.selectRange(tablePositionBase.getRow(), n3 - 1);
        } else {
            tableSelectionModel.selectRange(tablePositionBase.getRow(), n3 + 1);
        }
    }
    ...
    protected void updateCellVerticalSelection(int n2, Runnable runnable) {
        ...
        if (this.isShiftDown && tableSelectionModel.isSelected(n3 + n2, (TableColumnBase<T, ?>)tablePositionBase.getTableColumn())) {
            ...
            if (this.selectionHistory.size() >= 2) {
                ...
                boolean bl2 = bl = tablePositionBase2.getRow() == n4 && tablePositionBase2.getColumn() == tablePositionBase.getColumn();
            }
            int n5 = this.selectionPathDeviated ? (bl ? n3 : n4) : n3;
            tableSelectionModel.clearSelection(n5, (TableColumnBase<T, ?>)tablePositionBase.getTableColumn());
            tableFocusModel.focus(n4, tablePositionBase.getTableColumn());
        } else if (this.isShiftDown && this.getAnchor() != null && !this.selectionPathDeviated) {
            int n6 = tableFocusModel.getFocusedIndex() + n2;
            n6 = Math.max(Math.min(this.getItemCount() - 1, n6), 0);
            int n7 = Math.min(this.getAnchor().getRow(), n6);
            int n8 = Math.max(this.getAnchor().getRow(), n6);
            if (tableSelectionModel.getSelectedIndices().size() > 1) {
                this.clearSelectionOutsideRange(n7, n8, (TableColumnBase<T, ?>)tablePositionBase.getTableColumn());
            }
            for (int i2 = n7; i2 <= n8; ++i2) {
                if (tableSelectionModel.isSelected(i2, (TableColumnBase<T, ?>)tablePositionBase.getTableColumn())) continue;
                tableSelectionModel.select(i2, (TableColumnBase<T, ?>)tablePositionBase.getTableColumn());
            }
            tableFocusModel.focus(n6, tablePositionBase.getTableColumn());
        } else {
            int n9 = tableFocusModel.getFocusedIndex();
            if (!tableSelectionModel.isSelected(n9, (TableColumnBase<T, ?>)tablePositionBase.getTableColumn())) {
                tableSelectionModel.select(n9, (TableColumnBase<T, ?>)tablePositionBase.getTableColumn());
            }
            runnable.run();
        }
    }
    ...
    protected void updateCellHorizontalSelection(int n2, Runnable runnable) {
        ...
        boolean bl = false;
        TableColumnBase tableColumnBase = this.getColumn((TableColumnBase)tablePositionBase.getTableColumn(), n2);
        if (tableColumnBase == null) {
            tableColumnBase = tablePositionBase.getTableColumn();
            bl = true;
        }
        int n3 = tablePositionBase.getRow();
        if (this.isShiftDown && tableSelectionModel.isSelected(n3, tableColumnBase)) {
            Object object;
            if (bl) {
                return;
            }
            ...
            if (observableList.size() >= 2) {
                ...
                boolean bl3 = bl2 = ((TablePositionBase)object).getRow() == n3 && ((TablePositionBase)object).getTableColumn().equals(tableColumnBase);
            }
            object = this.selectionPathDeviated ? (bl2 ? tablePositionBase.getTableColumn() : tableColumnBase) : tablePositionBase.getTableColumn();
            tableSelectionModel.clearSelection(n3, (TableColumnBase<T, ?>)object);
            tableFocusModel.focus(n3, tableColumnBase);
        } else if (this.isShiftDown && this.getAnchor() != null && !this.selectionPathDeviated) {
            int n4 = this.getAnchor().getColumn();
            int n5 = this.getVisibleLeafIndex((TableColumnBase)tablePositionBase.getTableColumn()) + n2;
            n5 = Math.max(Math.min(this.getVisibleLeafColumns().size() - 1, n5), 0);
            int n6 = Math.min(n4, n5);
            int n7 = Math.max(n4, n5);
            for (int i2 = n6; i2 <= n7; ++i2) {
                tableSelectionModel.select(tablePositionBase.getRow(), this.getColumn(i2));
            }
            tableFocusModel.focus(tablePositionBase.getRow(), this.getColumn(n5));
        } else {
            runnable.run();
        }
    }
    ...
    protected TableColumnBase getColumn(TableColumnBase tableColumnBase, int n2) {
        return this.getVisibleLeafColumn(this.getVisibleLeafIndex(tableColumnBase) + n2);
    }
    ...
    protected void selectCell(int n2, int n3) {
        ...
        int n5 = this.getVisibleLeafIndex((TableColumnBase)tablePositionBase.getTableColumn());
        if (n2 < 0 && n4 <= 0) {
            return;
        }
        ...
        object = this.getColumn((TableColumnBase)object, n3);
        int n6 = tablePositionBase.getRow() + n2;
        tableSelectionModel.clearAndSelect(n6, (TableColumnBase<T, ?>)object);
        this.setAnchor(n6, (TableColumnBase)object);
    }
    ...
    protected void activate() {
        ...
        if (tablePositionBase.getRow() >= 0) {
            this.editCell(tablePositionBase.getRow(), (TableColumnBase)tablePositionBase.getTableColumn());
        }
    }
    ...
    protected void selectAllToFocus(boolean bl) {
        ...
        int n3 = tablePositionBase2.getRow();
        tableSelectionModel.clearSelection();
        if (!tableSelectionModel.isCellSelectionEnabled()) {
            int n4 = n3;
            int n5 = n3 > n2 ? n2 - 1 : n2 + 1;
            tableSelectionModel.selectRange(n4, n5);
        } else {
    ...
    protected void discontinuousSelectPreviousRow() {
        ...
        if (!tableSelectionModel.isCellSelectionEnabled()) {
            ...
            for (int i2 = n3; i2 < n4 + 1; ++i2) {
                tableSelectionModel.select(i2, tableColumnBase);
            }
            tableFocusModel.focus(n3, tableColumnBase);
        }
        if (this.onFocusPreviousRow != null) {
    ...
    protected void discontinuousSelectAllToLastRow() {
        ...
        if (!tableSelectionModel.isCellSelectionEnabled()) {
            ...
            for (int i2 = n2; i2 < this.getItemCount(); ++i2) {
                tableSelectionModel.select(i2, (TableColumnBase<T, ?>)this.getFocusedCell().getTableColumn());
            }
        }
        if (this.onMoveToLastCell != null) {
