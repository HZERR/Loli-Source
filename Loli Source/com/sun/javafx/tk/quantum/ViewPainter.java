...
implements Runnable {
    ...
    protected void paintImpl(Graphics graphics) {
        ...
        float f2 = this.getPixelScaleFactor();
        graphics2.setPixelScaleFactor(f2);
        ...
        if (bl && !this.sceneState.getScene().getDepthBuffer()) {
            n4 = (int)Math.ceil((float)this.width * f2);
            n3 = (int)Math.ceil((float)this.height * f2);
            if (this.sceneBuffer != null) {
                this.sceneBuffer.lock();
                if (this.sceneBuffer.isSurfaceLost() || n4 != this.sceneBuffer.getContentWidth() || n3 != this.sceneBuffer.getContentHeight()) {
            ...
            if (this.sceneBuffer == null) {
                this.sceneBuffer = graphics2.getResourceFactory().createRTTexture(n4, n3, Texture.WrapMode.CLAMP_TO_ZERO, false);
                bl2 = true;
            }
            this.sceneBuffer.contentsUseful();
            graphics2 = this.sceneBuffer.createGraphics();
            graphics2.scale(f2, f2);
        ...
        if (!bl2) {
            ...
            n4 = this.root.accumulateDirtyRegions(this.clip, this.dirtyRegionTemp, this.dirtyRegionPool, this.dirtyRegionContainer, this.tx, this.projTx);
            this.dirtyRegionContainer.roundOut();
            if (n4 == 1) {
                this.root.doPreCulling(this.dirtyRegionContainer, this.tx, this.projTx);
            }
        }
        int n5 = n3 = n4 == 1 ? this.dirtyRegionContainer.size() : 0;
        if (n3 > 0) {
            int n6;
            ...
            if (PulseLogger.PULSE_LOGGING_ENABLED) {
                PulseLogger.addMessage(n3 + " different dirty regions to render");
                for (n6 = 0; n6 < n3; ++n6) {
                    PulseLogger.addMessage("Dirty Region " + n6 + ": " + this.dirtyRegionContainer.getDirtyRegion(n6));
                    PulseLogger.addMessage("Render Root Path " + n6 + ": " + ViewPainter.getRootPath(n6));
                }
            }
            if (PulseLogger.PULSE_LOGGING_ENABLED && PrismSettings.printRenderGraph) {
                ...
                for (int i2 = 0; i2 < n3; ++i2) {
                    NodePath nodePath;
                    RectBounds rectBounds = this.dirtyRegionContainer.getDirtyRegion(i2);
                    if (!(rectBounds.getWidth() > 0.0f) || !(rectBounds.getHeight() > 0.0f) || (nodePath = ViewPainter.getRootPath(i2)).isEmpty()) continue;
            ...
            for (n2 = 0; n2 < n3; ++n2) {
                object = this.dirtyRegionContainer.getDirtyRegion(n2);
                if (!(((RectBounds)object).getWidth() > 0.0f) || !(((RectBounds)object).getHeight() > 0.0f)) continue;
                this.dirtyRect.setBounds((BaseBounds)object);
                if (f2 != 1.0f) {
                    this.dirtyRect.x = (int)((float)this.dirtyRect.x * f2);
                    this.dirtyRect.y = (int)((float)this.dirtyRect.y * f2);
                    this.dirtyRect.width = (int)((float)this.dirtyRect.width * f2);
                    this.dirtyRect.height = (int)((float)this.dirtyRect.height * f2);
                }
                graphics2.setClipRect(this.dirtyRect);
                graphics2.setClipRectIndex(n2);
                this.doPaint(graphics2, ViewPainter.getRootPath(n2));
            }
        } else {
        ...
        if (bl) {
            ...
            if (PrismSettings.showOverdraw) {
                if (n3 > 0) {
                    for (n2 = 0; n2 < n3; ++n2) {
                        object = new Rectangle(this.dirtyRegionContainer.getDirtyRegion(n2));
                        graphics.setClipRectIndex(n2);
