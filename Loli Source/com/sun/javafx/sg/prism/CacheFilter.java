...
public class CacheFilter {
    ...
    private boolean needToRenderCache(BaseTransform baseTransform, double[] arrd, float f2) {
        ...
        double d4 = arrd[2];
        if (this.scaleHint) {
            if (this.cachedScaleX < (double)f2 || this.cachedScaleY < (double)f2) {
                return true;
            }
            if (this.rotateHint) {
                return false;
    ...
    void updateScreenXform(double[] arrd) {
        if (this.scaleHint) {
            if (this.rotateHint) {
                double d2 = arrd[0] / this.cachedScaleX;
                double d3 = arrd[1] / this.cachedScaleY;
                double d4 = arrd[2] - this.cachedRotate;
                this.screenXform.setToScale(d2, d3);
                this.screenXform.rotate(d4);
            } else {
                double d5 = arrd[0] / this.cachedScaleX;
                double d6 = arrd[1] / this.cachedScaleY;
                this.screenXform.setToScale(d5, d6);
            }
        } else if (this.rotateHint) {
            double d7 = arrd[2] - this.cachedRotate;
    ...
    double[] unmatrix(BaseTransform baseTransform) {
        ...
        double d2 = Math.signum(arrarrd[0][0]);
        double d3 = Math.signum(arrarrd[1][1]);
        double d4 = d2 * this.v2length(arrarrd[0]);
        this.v2scale(arrarrd[0], d2);
        double d5 = this.v2dot(arrarrd[0], arrarrd[1]);
        this.v2combine(arrarrd[1], arrarrd[0], arrarrd[1], 1.0, -d5);
        double d6 = d3 * this.v2length(arrarrd[1]);
        this.v2scale(arrarrd[1], d3);
        double d7 = arrarrd[1][0];
        double d8 = arrarrd[0][0];
        double d9 = 0.0;
        d9 = d7 >= 0.0 ? Math.acos(d8) : (d8 > 0.0 ? Math.PI * 2 + Math.asin(d7) : Math.PI + Math.acos(-d8));
        arrd[0] = d4;
        arrd[1] = d6;
        arrd[2] = d9;
        return arrd;
    ...
    void v2combine(double[] arrd, double[] arrd2, double[] arrd3, double d2, double d3) {
        arrd3[0] = d2 * arrd[0] + d3 * arrd2[0];
        arrd3[1] = d2 * arrd[1] + d3 * arrd2[1];
    }
    ...
    void v2scale(double[] arrd, double d2) {
        double d3 = this.v2length(arrd);
        if (d3 != 0.0) {
            arrd[0] = arrd[0] * (d2 / d3);
            arrd[1] = arrd[1] * (d2 / d3);
        }
    ...
    void render(Graphics graphics) {
        ...
        if (this.needToRenderCache(baseTransform, arrd, f2 = graphics.getAssociatedScreen().getRenderScale())) {
            ...
            if (this.scaleHint) {
                ...
                this.cachedScaleY = Math.max((double)f2, arrd[1]);
        ...
        if (object == null) {
            ...
            this.impl_renderCacheToScreen(graphics, (Filterable)object, d2, d3);
            object.unlock();
        }
    }
    ...
    void impl_renderCacheToScreen(Object object, Filterable filterable, double d2, double d3) {
        Graphics graphics = (Graphics)object;
        graphics.setTransform(this.screenXform.getMxx(), this.screenXform.getMyx(), this.screenXform.getMxy(), this.screenXform.getMyy(), d2, d3);
        graphics.translate((float)this.cachedX, (float)this.cachedY);
        Object t2 = ((PrDrawable)filterable).getTextureObject();
        Rectangle rectangle = this.cachedImageData.getUntransformedBounds();
        graphics.drawTexture((Texture)t2, 0.0f, 0.0f, rectangle.width, rectangle.height);
    }
    ...
    boolean impl_scrollCacheCapable() {
        ...
        if (this.node instanceof NGRegion) {
            ...
            if (!background.isEmpty()) {
                ...
                return paint.isOpaque() && paint instanceof Color && backgroundFill.getInsets().equals(Insets.EMPTY) && baseBounds.getMinX() == 0.0f && baseBounds.getMinY() == 0.0f && baseBounds.getMaxX() == nGRegion.getWidth() && baseBounds.getMaxY() == nGRegion.getHeight();
            }
        }
        return true;
    }

    void impl_moveCacheBy(ImageData imageData, double d2, double d3) {
        PrDrawable prDrawable = (PrDrawable)imageData.getUntransformedImage();
        ...
        int n6 = rectangle.width - (int)Math.abs(d2);
        int n7 = rectangle.height - (int)Math.abs(d3);
        Graphics graphics = prDrawable.createGraphics();
        if (this.tempTexture != null) {
