...
implements PathIterator {
    ...
    public FlatteningPathIterator(PathIterator pathIterator, float f2) {
        this(pathIterator, f2, 10);
    }

    public FlatteningPathIterator(PathIterator pathIterator, float f2, int n2) {
        if (f2 < 0.0f) {
        ...
        this.squareflat = f2 * f2;
        this.limit = n2;
        this.levels = new int[n2 + 1];
        this.next(false);
    }
    ...
    void ensureHoldCapacity(int n2) {
        if (this.holdIndex - n2 < 0) {
            int n3 = this.hold.length - this.holdIndex;
            int n4 = this.hold.length + 24;
            float[] arrf = new float[n4];
            System.arraycopy(this.hold, this.holdIndex, arrf, this.holdIndex + 24, n3);
            this.hold = arrf;
            this.holdIndex += 24;
            this.holdEnd += 24;
        }
    ...
    private void next(boolean bl) {
        ...
        switch (this.holdType) {
            ...
            case 2: {
                ...
                int n2 = this.levels[this.levelIndex];
                while (n2 < this.limit && !(QuadCurve2D.getFlatnessSq(this.hold, this.holdIndex) < this.squareflat)) {
                    ...
                    this.holdIndex -= 4;
                    this.levels[this.levelIndex] = ++n2;
                    ++this.levelIndex;
                    this.levels[this.levelIndex] = n2;
                }
                this.holdIndex += 4;
            ...
            case 3: {
                ...
                int n3 = this.levels[this.levelIndex];
                while (n3 < this.limit && !(CubicCurve2D.getFlatnessSq(this.hold, this.holdIndex) < this.squareflat)) {
                    ...
                    this.holdIndex -= 6;
                    this.levels[this.levelIndex] = ++n3;
                    ++this.levelIndex;
                    this.levels[this.levelIndex] = n3;
                }
                this.holdIndex += 6;
