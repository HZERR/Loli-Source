...
extends BaseBounds {
    ...
    public BoxBounds(float f2, float f3, float f4, float f5, float f6, float f7) {
        this.setBounds(f2, f3, f4, f5, f6, f7);
    }
    ...
    public final void setBounds(float f2, float f3, float f4, float f5, float f6, float f7) {
        this.minX = f2;
        this.minY = f3;
        this.minZ = f4;
        this.maxX = f5;
    ...
    public void unionWith(float f2, float f3, float f4, float f5, float f6, float f7) {
        if (f5 < f2 || f6 < f3 || f7 < f4) {
            return;
        }
        if (this.isEmpty()) {
        ...
        this.minZ = Math.min(this.minZ, f4);
        this.maxX = Math.max(this.maxX, f5);
        this.maxY = Math.max(this.maxY, f6);
        this.maxZ = Math.max(this.maxZ, f7);
    }

    @Override
    public void add(float f2, float f3, float f4) {
        this.unionWith(f2, f3, f4, f2, f3, f4);
    }
    ...
    public void intersectWith(Rectangle rectangle) {
        ...
        this.intersectWith(f2, f3, 0.0f, f2 + (float)rectangle.width, f3 + (float)rectangle.height, 0.0f);
    }
    ...
    public void intersectWith(float f2, float f3, float f4, float f5, float f6, float f7) {
        ...
        this.minZ = Math.max(this.minZ, f4);
        this.maxX = Math.min(this.maxX, f5);
        this.maxY = Math.min(this.maxY, f6);
        this.maxZ = Math.min(this.maxZ, f7);
    }
    ...
    public boolean contains(float f2, float f3) {
        ...
        return this.contains(f2, f3, 0.0f);
    }
    ...
    public boolean contains(float f2, float f3, float f4) {
        ...
        return f2 >= this.minX && f2 <= this.maxX && f3 >= this.minY && f3 <= this.maxY && f4 >= this.minZ && f4 <= this.maxZ;
    }

    public boolean contains(float f2, float f3, float f4, float f5, float f6, float f7) {
        if (this.isEmpty()) {
            return false;
        }
        return this.contains(f2, f3, f4) && this.contains(f2 + f5, f3 + f6, f4 + f7);
    }

    @Override
    public boolean intersects(float f2, float f3, float f4, float f5) {
        return this.intersects(f2, f3, 0.0f, f4, f5, 0.0f);
    }
    ...
    public boolean intersects(float f2, float f3, float f4, float f5, float f6, float f7) {
        ...
        return f2 + f5 >= this.minX && f3 + f6 >= this.minY && f4 + f7 >= this.minZ && f2 <= this.maxX && f3 <= this.maxY && f4 <= this.maxZ;
    }
    ...
    public boolean disjoint(float f2, float f3, float f4, float f5) {
        return this.disjoint(f2, f3, 0.0f, f4, f5, 0.0f);
    }

    public boolean disjoint(float f2, float f3, float f4, float f5, float f6, float f7) {
        if (this.isEmpty()) {
            return true;
        }
        return f2 + f5 < this.minX || f3 + f6 < this.minY || f4 + f7 < this.minZ || f2 > this.maxX || f3 > this.maxY || f4 > this.maxZ;
    ...
    public void grow(float f2, float f3, float f4) {
        this.minX -= f2;
        this.maxX += f2;
        this.minY -= f3;
        this.maxY += f3;
    ...
    public int hashCode() {
        ...
        n2 = 79 * n2 + Float.floatToIntBits(this.minZ);
        n2 = 79 * n2 + Float.floatToIntBits(this.maxX);
        n2 = 79 * n2 + Float.floatToIntBits(this.maxY);
        n2 = 79 * n2 + Float.floatToIntBits(this.maxZ);
        return n2;
    }
