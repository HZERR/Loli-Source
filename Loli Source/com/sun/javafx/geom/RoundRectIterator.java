...
implements PathIterator {
    ...
    public int currentSegment(float[] arrf) {
        ...
        int n2 = 0;
        for (int i2 = 0; i2 < arrd.length; i2 += 4) {
            arrf[n2++] = (float)(this.x + arrd[i2 + 0] * this.w + arrd[i2 + 1] * this.aw);
            arrf[n2++] = (float)(this.y + arrd[i2 + 2] * this.h + arrd[i2 + 3] * this.ah);
        }
        if (this.transform != null) {
            this.transform.transform(arrf, 0, arrf, 0, n2 / 2);
        }
