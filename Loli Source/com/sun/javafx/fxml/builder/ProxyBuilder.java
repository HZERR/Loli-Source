...
implements Builder<T> {
    ...
    private final Comparator<Constructor> constructorComparator = (constructor, constructor2) -> {
        ...
        for (int i2 = 0; i2 < n4; ++i2) {
            Class<?> class_;
            Class<?> class_2 = constructor.getParameterTypes()[i2];
            if (class_2.equals(class_ = constructor2.getParameterTypes()[i2])) continue;
    ...
    public ProxyBuilder(Class<?> class_) {
        ...
        for (Constructor<?> constructor3 : arrconstructor = ConstructorUtil.getConstructors(this.type)) {
            ...
            for (Class<?> class_2 : arrclass) {
                ...
                for (Annotation annotation : arrannotation[n2]) {
    ...
    private Object createObjectFromConstructor(Constructor constructor, Set<String> set) {
        ...
        for (AnnotationValue object3 : map.values()) {
            ...
            if (object != null) {
                try {
                    arrobject[n2] = BeanAdapter.coerce(object, object3.getType());
                }
                catch (Exception exception) {
                    return null;
                }
                ...
                try {
                    arrobject[n2] = BeanAdapter.coerce(object3.getDefaultValue(), object3.getType());
                }
                catch (Exception exception) {
                    return null;
                }
            } else {
                arrobject[n2] = ProxyBuilder.getDefaultValue(object3.getType());
            }
    ...
    private Object createObjectWithExactArguments(Constructor constructor, Set<String> set) {
        ...
        for (String string : set) {
            ...
            try {
                arrobject[n2++] = BeanAdapter.coerce(object2, class_);
            }
            catch (Exception exception) {
                return null;
            }
