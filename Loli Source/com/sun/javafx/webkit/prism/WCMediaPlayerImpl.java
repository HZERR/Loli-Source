...
PlayerTimeListener {
            ...
            Thread thread = new Thread(new Runnable(){
                ...
                public void run() {
                    ...
                    while (WCMediaPlayerImpl.this.isSeeking() && (mediaPlayer = WCMediaPlayerImpl.this.getPlayer()) != null) {
                        double d2 = mediaPlayer.getPresentationTime();
                        if ((double)f3 < 0.01 || Math.abs(d2) >= 0.01) {
    ...
    private void renderImpl(WCGraphicsContext wCGraphicsContext, int n2, int n3, int n4, int n5) {
        ...
        if (texture != null) {
            ...
            wCGraphicsContext.fillRect(n2, n3, n4, n5, -16777216);
        }
        if (verbose) {
            log.log(Level.FINER, "<<(Prism)renderImpl");
        }
    ...
    public void onFinish(PlayerStateEvent playerStateEvent) {
        MediaPlayer mediaPlayer = this.getPlayer();
        if (mediaPlayer != null) {
            int n2 = this.finished = mediaPlayer.getRate() > 0.0f ? 1 : -1;
            if (verbose) {
    ...
    public void onError(Object object, int n2, String string) {
        if (verbose) {
            log.log(Level.WARNING, "onError, errCode={0}, msg={1}", new Object[]{n2, string});
        }
    ...
    public void onDurationChanged(double d2) {
        if (verbose) {
            log.log(Level.FINE, "onDurationChanged, duration={0}", d2);
        }
    ...
    public void onSizeChanged(int n2, int n3) {
        if (verbose) {
            log.log(Level.FINE, "onSizeChanged, new size = {0} x {1}", new Object[]{n2, n3});
        }
    ...
    private void updateBufferingStatus() {
        int n2;
        int n3 = this.buffering ? 2 : (n2 = this.bufferedStart > 0.0f ? 1 : 3);
        if (verbose) {
    ...
    public void onBufferProgress(BufferProgressEvent bufferProgressEvent) {
        ...
        if (verbose) {
            log.log(Level.FINER, "onBufferProgress, bufferStart={0}, bufferStop={1}, bufferPos={2}, duration={3}; notify range [{4},[5]], bytesLoaded: {6}", new Object[]{bufferProgressEvent.getBufferStart(), bufferProgressEvent.getBufferStop(), bufferProgressEvent.getBufferPosition(), bufferProgressEvent.getDuration(), Float.valueOf(arrf[0]), Float.valueOf(arrf[1]), n2});
        }
        this.notifyBufferChanged(arrf, n2);
