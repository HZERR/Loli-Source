...
extends WCGradient<RadialGradient> {
    static final Comparator<Stop> COMPARATOR = (stop, stop2) -> {
        float f2;
        float f3 = stop.getOffset();
        if (f3 < (f2 = stop2.getOffset())) {
            return -1;
        }
        if (f3 > f2) {
            return 1;
    ...
    WCRadialGradient(WCPoint wCPoint, float f2, WCPoint wCPoint2, float f3) {
        this.reverse = f2 < f3;
        this.p1 = this.reverse ? wCPoint2 : wCPoint;
        this.p2 = this.reverse ? wCPoint : wCPoint2;
        this.r1 = this.reverse ? f3 : f2;
        this.r2 = this.reverse ? f2 : f3;
        this.r1over = this.r1 > 0.0f ? 1.0f / this.r1 : 0.0f;
    }

    @Override
    protected void addStop(int n2, float f2) {
        if (this.reverse) {
            f2 = 1.0f - f2;
        }
        f2 = 1.0f - f2 + f2 * this.r2 * this.r1over;
        this.stops.add(new Stop(WCGraphicsPrismContext.createColor(n2), f2));
    }

    @Override
    public RadialGradient getPlatformGradient() {
        Collections.sort(this.stops, COMPARATOR);
        float f2 = this.p2.getX() - this.p1.getX();
        float f3 = this.p2.getY() - this.p1.getY();
        return new RadialGradient(this.p1.getX(), this.p1.getY(), (float)(Math.atan2(f3, f2) * 180.0 / Math.PI), (float)Math.sqrt(f2 * f2 + f3 * f3) * this.r1over, this.r1, BaseTransform.IDENTITY_TRANSFORM, this.isProportional(), this.getSpreadMethod() - 1, this.stops);
    }
    ...
    static String toString(WCGradient wCGradient, WCPoint wCPoint, WCPoint wCPoint2, Float f2, List<Stop> list) {
        ...
        if (f2 != null) {
            stringBuilder.append(", radius=").append(f2);
        }
        ...
        for (int i2 = 0; i2 < list.size(); ++i2) {
            ...
            stringBuilder.append(list.get(i2).getOffset()).append(":").append(list.get(i2).getColor());
        }
