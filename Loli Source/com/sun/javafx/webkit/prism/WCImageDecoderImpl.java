...
extends WCImageDecoder {
    ...
    private final ImageLoadListener readerListener = new ImageLoadListener(){
        ...
        @Override
        public void imageLoadProgress(ImageLoader imageLoader, float f2) {
        }
    ...
    protected void addImageData(byte[] arrby) {
        if (arrby != null) {
            ...
            if (this.data == null) {
                ...
                int n2 = this.dataSize + arrby.length;
    ...
    @Override
    protected WCImageFrame getFrame(int n2, int[] arrn) {
        ...
        if (imageFrame != null) {
            ...
            if (arrn != null) {
                ...
                arrn[0] = n2 < this.frames.length - 1 ? 1 : 0;
                arrn[1] = ((WCImage)object).getWidth();
                arrn[2] = ((WCImage)object).getHeight();
                arrn[3] = n3;
                arrn[4] = 1;
                if (log.isLoggable(Level.FINE)) {
                    log.fine(String.format("%X getFrame(%d): complete=%d, size=%dx%d, duration=%d, hasAlpha=%d", this.hashCode(), n2, arrn[0], arrn[1], arrn[2], arrn[3], arrn[4]));
                }
            }
    ...
    private ImageFrame getImageFrame(int n2) {
        if (!this.fullDataReceived) {
            this.startLoader();
    ...
    private PrismImage getPrismImage(int n2, ImageFrame imageFrame) {
        if (this.images == null) {
