...
MarkerStateListener {
    ...
    protected void sendWarning(int n2, String string) {
        if (this.eventLoop != null) {
            String string2 = String.format("Internal media warning: %d", n2);
            if (string != null) {
    ...
    protected void sendPlayerMediaErrorEvent(int n2) {
        this.sendPlayerEvent(new MediaErrorEvent(this, MediaError.getFromCode(n2)));
    }
    ...
    protected void sendFrameSizeChangedEvent(int n2, int n3) {
        this.sendPlayerEvent(new FrameSizeChangedEvent(n2, n3));
    }

    protected void sendAudioTrack(boolean bl, long l2, String string, int n2, String string2, int n3, int n4, float f2) {
        Locale locale = null;
        if (!string2.equals("und")) {
            locale = new Locale(string2);
        }
        AudioTrack audioTrack = new AudioTrack(bl, l2, string, locale, Track.Encoding.toEncoding(n2), n3, n4, f2);
        TrackEvent trackEvent = new TrackEvent(audioTrack);
        this.sendPlayerEvent(trackEvent);
    }

    protected void sendVideoTrack(boolean bl, long l2, String string, int n2, int n3, int n4, float f2, boolean bl2) {
        VideoTrack videoTrack = new VideoTrack(bl, l2, string, null, Track.Encoding.toEncoding(n2), new VideoResolution(n3, n4), f2, bl2);
        TrackEvent trackEvent = new TrackEvent(videoTrack);
        this.sendPlayerEvent(trackEvent);
    }

    protected void sendSubtitleTrack(boolean bl, long l2, String string, int n2, String string2) {
        Locale locale = null;
        if (null != string2) {
            locale = new Locale(string2);
        }
        SubtitleTrack subtitleTrack = new SubtitleTrack(bl, l2, string, locale, Track.Encoding.toEncoding(n2));
    ...
    protected void sendDurationUpdateEvent(double d2) {
        this.sendPlayerEvent(new PlayerTimeEvent(d2));
    }
    ...
    protected void sendAudioSpectrumEvent(double d2, double d3) {
        this.sendPlayerEvent(new AudioSpectrumEvent(this.getAudioSpectrum(), d2, d3));
    }
...
lbl22:
        ...
        private void HandleRendererEvents(NewFrameEvent newFrameEvent) {
            ...
            if (!NativeMediaPlayer.this.videoFrameRateListeners.isEmpty()) {
                ...
                if (NativeMediaPlayer.this.recomputeFrameRate) {
                    ...
                    NativeMediaPlayer.this.previousFrameTime = d2;
                    NativeMediaPlayer.this.numFramesSincePlaying = 1L;
                } else {
                    boolean bl = false;
                    if (NativeMediaPlayer.this.numFramesSincePlaying == 1L) {
                        NativeMediaPlayer.this.meanFrameDuration = d2 - NativeMediaPlayer.this.previousFrameTime;
                        ...
                        double d3 = NativeMediaPlayer.this.meanFrameDuration;
                        int n2 = NativeMediaPlayer.this.encodedFrameRate != 0.0 ? (int)(NativeMediaPlayer.this.encodedFrameRate + 0.5) : 30;
                        long l2 = NativeMediaPlayer.this.numFramesSincePlaying < (long)n2 ? NativeMediaPlayer.this.numFramesSincePlaying : (long)n2;
                        NativeMediaPlayer.this.meanFrameDuration = ((double)(l2 - 1L) * d3 + d2 - NativeMediaPlayer.this.previousFrameTime) / (double)l2;
    ...
    extends PlayerEvent {
        ...
        public FrameSizeChangedEvent(int n2, int n3) {
    ...
    extends PlayerEvent {
        ...
        public PlayerTimeEvent(double d2) {
            this.time = d2;
        }
