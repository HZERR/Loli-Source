...
extends Media {
     ...
     */
    @Override
    public void addMarker(String string, double d2) {
        if (string == null) {
        ...
        try {
            ...
            this.markersByName.put(string, d2);
            this.markersByTime.put(d2, string);
        }
        finally {
    ...
    public double removeMarker(String string) {
        ...
        try {
            if (this.markersByName.containsKey(string)) {
                d2 = this.markersByName.get(string);
                this.markersByName.remove(string);
                this.markersByTime.remove(d2);
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    Map.Entry<Double, String> getNextMarker(double d2, boolean bl) {
        Map.Entry<Double, String> entry = null;
        this.markerLock.lock();
        try {
            if (this.markersByTime != null) {
                entry = bl ? this.markersByTime.ceilingEntry(d2) : this.markersByTime.higherEntry(d2);
            }
        }
        finally {
            this.markerLock.unlock();
