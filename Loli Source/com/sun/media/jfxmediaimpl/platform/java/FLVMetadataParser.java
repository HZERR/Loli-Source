...
extends MetadataParserImpl {
        ...
        block5: {
            try {
                ...
                for (n3 = 0; n3 < 10; ++n3) {
                    ...
                    this.skipBytes(7);
                    if (by == 18) {
                        int n4 = this.getStreamPosition() + this.dataSize;
                        if (!this.parseDataTag()) {
                            if (this.getStreamPosition() >= n4) continue;
                            this.skipBytes(n4 - this.getStreamPosition());
                            continue;
                        }
                        break;
                    }
    ...
    private boolean parseDataTag() throws IOException {
        ...
        for (n2 = 0; n2 < 14; ++n2) {
            arrby[n2] = this.getNextByte();
        }
        if (arrby[0] != 2) {
            return false;
        }
        n2 = (arrby[1] & 0xFF) << 8 | arrby[2] & 0xFF;
        if (n2 != 10) {
            return false;
        }
        ...
        String string = new String(arrby, 3, n2, Charset.forName(CHARSET_UTF_8));
        ...
        try {
            ...
            do {
                ...
                if (Logger.canLog(1) && !string2.equals("")) {
                    Logger.logMsg(1, n4 + ": \"" + string2 + "\" -> " + (null == string3 ? "(unsupported)" : "\"" + string3 + "\""));
                }
