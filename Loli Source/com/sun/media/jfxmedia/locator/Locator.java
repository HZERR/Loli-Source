...
public class Locator {
    ...
    public void cacheMedia() {
        ...
        if (null == cacheReference) {
            ...
            int n2 = 0;
            while ((long)n2 < this.contentLength) {
                int n3;
                try {
    ...
    public void init() throws URISyntaxException, IOException, FileNotFoundException {
        try {
            int n2;
            int n3 = this.uriString.indexOf("/");
            if (n3 != -1 && this.uriString.charAt(n3 + 1) != '/') {
            ...
            if (System.getProperty("os.name").toLowerCase().indexOf("win") == -1 && this.protocol.equals("file") && (n2 = this.uriString.indexOf("/~/")) != -1) {
                this.uriString = this.uriString.substring(0, n2) + System.getProperty("user.home") + this.uriString.substring(n2 + 2);
            }
    ...
    private String getContentTypeFromFileSignature(URI uRI) throws MalformedURLException, IOException {
        ...
        inputStream.close();
        return MediaUtils.fileSignatureToContentType(arrby, n2);
    }
