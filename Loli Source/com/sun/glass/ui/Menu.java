...
public final class Menu {
     ...
     */
    public void insert(Menu menu, int n2) throws IndexOutOfBoundsException {
        Application.checkEventThread();
        if (menu == null) {
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    public void insert(MenuItem menuItem, int n2) throws IndexOutOfBoundsException {
        Application.checkEventThread();
        List<Object> list = this.items;
        synchronized (list) {
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    public void remove(int n2) throws IndexOutOfBoundsException {
        Application.checkEventThread();
        List<Object> list = this.items;
        synchronized (list) {
            Object object = this.items.get(n2);
            boolean bl = false;
            bl = object == MenuItem.Separator ? this.delegate.remove((MenuItemDelegate)null, n2) : (object instanceof MenuItem ? this.delegate.remove(((MenuItem)object).getDelegate(), n2) : this.delegate.remove(((Menu)object).getDelegate(), n2));
            if (bl) {
    ...
    public static class EventHandler {
        public void handleMenuOpening(Menu menu, long l2) {
        }

        public void handleMenuClosed(Menu menu, long l2) {
        }
    }
}
