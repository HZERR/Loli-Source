...
public abstract class CursorManager<T> {
    ...
    private long getCustomCursorID(WCImageFrame wCImageFrame, int n2, int n3) {
        return this.putCursor(this.getCustomCursor(wCImageFrame.getFrame(), n2, n3));
    }

    private long getPredefinedCursorID(int n2) {
        return this.putCursor(this.getPredefinedCursor(n2));
    }

    public final T getCursor(long l2) {
        return this.map.get(l2);
    }

    private long putCursor(T t2) {
        long l2 = t2.hashCode();
        this.map.put(l2, t2);
        return l2;
    }
