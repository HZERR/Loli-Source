    ...
    extends PushbackInputStream {
        ...
        private DirectoryInputStream(InputStream inputStream, boolean bl) {
            ...
            if (bl) {
                ...
                int n2 = 0;
                try {
                    n2 = super.read(this.buffer, 0, this.buffer.length);
                }
                catch (IOException iOException) {
                    // empty catch block
                }
                if (n2 <= 0) {
                    ...
                    for (int i2 = 0; i2 < n2; ++i2) {
                        charSequence.append((char)this.buffer[i2]);
                    }
            ...
            if (DirectoryURLConnection.this.toHTML) {
                ...
                if (string != null && !string.isEmpty() && (n3 = string.lastIndexOf("/", string.length() - 2)) >= 0) {
                    int n4 = string.length() - n3 - 1;
                    n3 = DirectoryURLConnection.this.dirUrl.indexOf(string);
                    charSequence = DirectoryURLConnection.this.dirUrl.substring(0, n3 + string.length() - n4) + DirectoryURLConnection.this.dirUrl.substring(n3 + string.length());
                }
        ...
        private void parseFile(String string) {
            ...
            while ((n2 = this.tmpString.indexOf("\n")) >= 0) {
                String string2 = this.tmpString.substring(0, n2);
                this.tmpString.delete(0, n2 + 1);
        ...
        private void parseFTP(String string) {
            ...
            while ((n2 = this.tmpString.indexOf("\n")) >= 0) {
                ...
                for (int i2 = 0; i2 < patterns.length; ++i2) {
                    matcher = patterns[i2].matcher(string2);
                    if (!matcher.find()) continue;
                    string3 = matcher.group(patternGroups[i2][0]);
                    string5 = matcher.group(patternGroups[i2][1]);
                    string6 = matcher.group(patternGroups[i2][2]);
                    if (patternGroups[i2][3] > 0) {
                        string6 = string6 + " " + matcher.group(patternGroups[i2][3]);
                    }
                    if (patternGroups[i2][4] > 0) {
                        String string7 = matcher.group(patternGroups[i2][4]);
        ...
        public int read(byte[] arrby, int n2, int n3) throws IOException {
            ...
            if (this.bytesIn != null) {
                n4 = this.bytesIn.read(arrby, n2, n3);
                if (n4 == -1) {
                    this.bytesIn.close();
                    this.bytesIn = null;
                    if (this.endOfStream) {
            ...
            if (!this.endOfStream) {
                n4 = super.read(this.buffer, 0, this.buffer.length);
                if (n4 == -1) {
                ...
                if (DirectoryURLConnection.this.ftp) {
                    this.parseFTP(new String(this.buffer, 0, n4));
                } else {
                    this.parseFile(new String(this.buffer, 0, n4));
                }
                if (this.bytesIn != null) {
                    return this.read(arrby, n2, n3);
                }
