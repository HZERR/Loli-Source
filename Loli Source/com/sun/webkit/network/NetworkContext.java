...
final class NetworkContext {
    ...
    private static URLLoader fwkLoad(WebPage webPage, boolean bl, String string, String string2, String string3, FormDataElement[] arrformDataElement, long l2) {
        if (logger.isLoggable(Level.FINEST)) {
            logger.log(Level.FINEST, String.format("webPage: [%s], asynchronous: [%s], url: [%s], method: [%s], formDataElements: %s, data: [0x%016X], headers:%n%s", webPage, bl, string, string2, arrformDataElement != null ? Arrays.asList(arrformDataElement) : "[null]", l2, Util.formatHeaders(string3)));
        }
        URLLoader uRLLoader = new URLLoader(webPage, byteBufferPool, bl, string, string2, string3, arrformDataElement, l2);
        if (bl) {
    ...
    private static int fwkGetMaximumHTTPConnectionCountPerHost() {
        int n2 = AccessController.doPrivileged(() -> Integer.getInteger("http.maxConnections", -1));
        return n2 >= 0 ? n2 : 5;
    }
