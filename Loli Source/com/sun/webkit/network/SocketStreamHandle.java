...
final class SocketStreamHandle {
    ...
    private SocketStreamHandle(String string, int n2, boolean bl, WebPage webPage, long l2) {
        this.host = string;
        this.port = n2;
        this.ssl = bl;
    ...
    private static SocketStreamHandle fwkCreate(String string, int n2, boolean bl, WebPage webPage, long l2) {
        SocketStreamHandle socketStreamHandle = new SocketStreamHandle(string, n2, bl, webPage, l2);
    ...
    private void doRun() {
        ...
        try {
            ...
            while ((n2 = inputStream.read(arrby = new byte[8192])) > 0) {
                if (logger.isLoggable(Level.FINEST)) {
                    logger.log(Level.FINEST, String.format("%s received len: [%d], data:%s", this, n2, SocketStreamHandle.dump(arrby, n2)));
                }
                this.didReceiveData(arrby, n2);
            }
    ...
    private void didReceiveData(byte[] arrby, int n2) {
        Invoker.getInvoker().postOnEventThread(() -> {
            if (this.state == State.ACTIVE) {
                this.notifyDidReceiveData(arrby, n2);
            }
        });
    }
    ...
    private void notifyDidReceiveData(byte[] arrby, int n2) {
        if (logger.isLoggable(Level.FINEST)) {
            logger.log(Level.FINEST, String.format("%s, len: [%d], data:%s", this, n2, SocketStreamHandle.dump(arrby, n2)));
        }
        SocketStreamHandle.twkDidReceiveData(arrby, n2, this.data);
    }

    private void notifyDidFail(int n2, String string) {
        if (logger.isLoggable(Level.FINEST)) {
            logger.log(Level.FINEST, String.format("%s, errorCode: %d, errorDescription: %s", this, n2, string));
        }
        SocketStreamHandle.twkDidFail(n2, string, this.data);
    }
    ...
    private static String dump(byte[] arrby, int n2) {
        ...
        while (n3 < n2) {
            ...
            while (n4 < 16) {
                if (n3 < n2) {
                    int n5 = arrby[n3] & 0xFF;
                    stringBuilder2.append(String.format("%02x ", n5));
                    stringBuilder3.append(n5 >= 32 && n5 <= 126 ? (char)n5 : (char)'.');
                } else {
