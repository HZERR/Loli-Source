...
implements Runnable {
    ...
    URLLoader(WebPage webPage, ByteBufferPool byteBufferPool, boolean bl, String string, String string2, String string3, FormDataElement[] arrformDataElement, long l2) {
    ...
    private void doRun() {
        ...
        try {
            ...
            while (true) {
                ...
                if (this.url.startsWith("file:") && (n4 = this.url.indexOf(63)) != -1) {
                    string = this.url.substring(0, n4);
                }
    ...
    private void prepareConnection(URLConnection uRLConnection) throws IOException {
        ...
        if (this.headers != null && this.headers.length() > 0) {
            for (String string2 : this.headers.split("\n")) {
                ...
                if (n2 <= 0) continue;
                uRLConnection.addRequestProperty(string2.substring(0, n2), string2.substring(n2 + 2));
            }
        }
        ...
        try {
            ...
            if (n2 != 0) {
                ...
                for (FormDataElement formDataElement : this.formDataElements) {
                    ...
                    while ((n5 = inputStream.read(arrby)) > 0) {
                        outputStream.write(arrby, 0, n5);
                ...
                if (uRLConnection instanceof HttpURLConnection) {
                    ...
                    switch (n2) {
                        ...
                        case 307: {
                            ...
                            return new Redirect(uRL.toExternalForm(), n2 == 307);
                        }
                        case 304: {
        ...
        try {
            if (object3 != null) {
                ...
                while (!this.canceled) {
                    ...
                    int n4;
                    try {
                        n4 = ((InputStream)object3).read(arrby);
                    }
                    ...
                    if (n4 < (n3 = ((Buffer)object).remaining())) {
                        ((ByteBuffer)object).put(arrby, 0, n4);
                        continue;
                    }
                    ((ByteBuffer)object).put(arrby, 0, n3);
                    ((Buffer)object).flip();
                    ...
                    if (n5 <= 0) continue;
                    object = object2.allocate();
                    ((ByteBuffer)object).put(arrby, n3, n5);
                }
            }
            if (!this.canceled) {
    ...
    private void notifyDidSendData(long l2, long l3) {
        if (logger.isLoggable(Level.FINEST)) {
            logger.log(Level.FINEST, String.format("totalBytesSent: [%d], totalBytesToBeSent: [%d], data: [0x%016X]", l2, l3, this.data));
        }
    ...
    private void willSendRequest(String string, String string2, URLConnection uRLConnection) throws InterruptedException {
        ...
        this.callBack(() -> {
            try {
                boolean bl;
                if (!this.canceled && !(bl = this.notifyWillSendRequest(string3, string2, n2, string4, string5, l2, string6, string7))) {
                    this.fwkCancel();
                }
            }
            finally {
    ...
    private boolean notifyWillSendRequest(String string, String string2, int n2, String string3, String string4, long l2, String string5, String string6) {
        if (logger.isLoggable(Level.FINEST)) {
            logger.log(Level.FINEST, String.format("newUrl: [%s], newMethod: [%s], status: [%d], contentType: [%s], contentEncoding: [%s], contentLength: [%d], url: [%s], data: [0x%016X], headers:%n%s", string, string2, n2, string3, string4, l2, string6, this.data, Util.formatHeaders(string5)));
        }
        boolean bl = URLLoader.twkWillSendRequest(string, string2, n2, string3, string4, l2, string5, string6, this.data);
        if (logger.isLoggable(Level.FINEST)) {
    ...
    private void notifyDidReceiveResponse(int n2, String string, String string2, long l2, String string3, String string4) {
        if (logger.isLoggable(Level.FINEST)) {
            logger.log(Level.FINEST, String.format("status: [%d], contentType: [%s], contentEncoding: [%s], contentLength: [%d], url: [%s], data: [0x%016X], headers:%n%s", n2, string, string2, l2, string4, this.data, Util.formatHeaders(string3)));
        }
    ...
    private void notifyDidReceiveData(ByteBuffer byteBuffer, int n2, int n3) {
        if (logger.isLoggable(Level.FINEST)) {
            logger.log(Level.FINEST, String.format("byteBuffer: [%s], position: [%s], remaining: [%s], data: [0x%016X]", byteBuffer, n2, n3, this.data));
        }
    ...
    private void notifyDidFail(int n2, String string, String string2) {
        if (logger.isLoggable(Level.FINEST)) {
            logger.log(Level.FINEST, String.format("errorCode: [%d], url: [%s], message: [%s], data: [0x%016X]", n2, string, string2, this.data));
        }
