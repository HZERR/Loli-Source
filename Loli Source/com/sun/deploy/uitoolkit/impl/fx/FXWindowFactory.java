...
extends PluginWindowFactory {
    ...
    public Window createWindow(final long l2, final String string, boolean bl, ModalityInterface modalityInterface) {
        try {
            return FXPluginToolkit.callAndWait(new Callable<FXWindow>(){

                @Override
                public FXWindow call() throws Exception {
                    return new FXWindow(l2, string);
                }
            });
        }
    ...
    public Window createWindow(long l2, String string, boolean bl, ModalityInterface modalityInterface, Pipe pipe, int n2) {
        return this.createWindow(l2, string, bl, modalityInterface);
    }
}
