...
implements ConsoleWindow {
    ...
    public FXConsole(ConsoleController consoleController) {
        ...
        FXConsole.invokeLater(new Runnable(){
            ...
            public void run() {
                ...
                FXConsole.this.dialog.getScene().setOnKeyTyped((EventHandler<? super KeyEvent>)eventHandler);
                FXConsole.this.textArea.setOnKeyTyped((EventHandler<? super KeyEvent>)eventHandler);
    ...
    private void copyConsole() {
        int n2 = this.textArea.getSelection().getStart();
        int n3 = this.textArea.getSelection().getEnd();
        if (n3 - n2 <= 0) {
    ...
    private void showMemory() {
        ...
        long l4 = (long)(100.0 / ((double)l3 / (double)l2));
        MessageFormat messageFormat = new MessageFormat(ResourceManager.getMessage("console.memory"));
        Object[] arrobject = new Object[]{new Long(l3), new Long(l2), new Long(l4)};
    ...
    public void append(final String string) {
        FXConsole.invokeLater(new Runnable(){
            ...
            public void run() {
                ...
                int n2 = FXConsole.this.textArea.getText().length();
                if (n2 > 1) {
                    FXConsole.this.textArea.insertText(n2, string);
                } else {
    ...
    private void setScrollPosition() {
        ...
        if (scrollBar != null) {
            double d2 = scrollBar.getMax();
            double d3 = scrollBar.getValue();
            if (d3 < d2) {
                scrollBar.setValue(d2);
            }
        }
    }
