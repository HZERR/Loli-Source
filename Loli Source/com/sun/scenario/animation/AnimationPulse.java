...
implements AnimationPulseMBean {
    ...
    public void recordStart(long l2) {
        ...
        this.pulseData = new PulseData(TimeUnit.MILLISECONDS.toNanos(l2));
    }
    ...
    private long getAv(PulseData.Accessor accessor, long l2, TimeUnit timeUnit) {
        if (!this.getEnabled()) {
            return 0L;
        }
        long l3 = 0L;
        long l4 = 0L;
        for (PulseData pulseData : this.pulseDataQueue) {
            if (pulseData.getPulseStartFromNow(timeUnit) > l2) continue;
            l3 += accessor.get(pulseData, timeUnit);
            ++l4;
        }
        return l4 == 0L ? 0L : l3 / l4;
    }

    private long getMax(PulseData.Accessor accessor, long l2, TimeUnit timeUnit) {
        if (!this.getEnabled()) {
            return 0L;
        }
        long l3 = 0L;
        for (PulseData pulseData : this.pulseDataQueue) {
            if (pulseData.getPulseStartFromNow(timeUnit) > l2) continue;
            l3 = Math.max(accessor.get(pulseData, timeUnit), l3);
        }
        return l3;
    }
