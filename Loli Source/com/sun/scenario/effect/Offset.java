...
extends Effect {
    ...
    public Offset(int n2, int n3, Effect effect) {
        super(effect);
        this.xoff = n2;
        this.yoff = n3;
    }
    ...
    static BaseTransform getOffsetTransform(BaseTransform baseTransform, double d2, double d3) {
        if (baseTransform == null || baseTransform.isIdentity()) {
            return Translate2D.getInstance(d2, d3);
        }
        return baseTransform.copy().deriveWithTranslation(d2, d3);
    }
    ...
    public Point2D transform(Point2D point2D, Effect effect) {
        ...
        float f2 = point2D.x + (float)this.xoff;
        float f3 = point2D.y + (float)this.yoff;
        point2D = new Point2D(f2, f3);
        return point2D;
    }
    ...
    public DirtyRegionContainer getDirtyRegions(Effect effect, DirtyRegionPool dirtyRegionPool) {
        ...
        if (this.xoff != 0 || this.yoff != 0) {
            for (int i2 = 0; i2 < dirtyRegionContainer.size(); ++i2) {
                dirtyRegionContainer.getDirtyRegion(i2).translate(this.xoff, this.yoff, 0.0f);
            }
        }
