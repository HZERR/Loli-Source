...
public abstract class Renderer {
    ...
    public PoolFilterable getCompatibleImage(int n2, int n3) {
        return this.imagePool.checkOut(this, n2, n3);
    }
    ...
    public ImageData transform(FilterContext filterContext, ImageData imageData, int n2, int n3) {
        double d2;
        ...
        while (n2 < -1 || n3 < -1) {
            ...
            affine2D.setToScale(d2, d3);
            imageData = this.transform(filterContext, imageData, (BaseTransform)affine2D, rectangle, rectangle2);
        }
        if ((n2 | n3) != 0) {
            double d4 = n2 < 0 ? 0.5 : (double)(1 << n2);
            d2 = n3 < 0 ? 0.5 : (double)(1 << n3);
            affine2D.setToScale(d4, d2);
    ...
    public final synchronized EffectPeer getPeerInstance(FilterContext filterContext, String string, int n2) {
        EffectPeer effectPeer = this.peerCache.get(string);
        if (effectPeer != null) {
            return effectPeer;
        }
        if (n2 > 0 && (effectPeer = this.peerCache.get(string + "_" + n2)) != null) {
            return effectPeer;
        }
        effectPeer = this.createPeer(filterContext, string, n2);
        if (effectPeer == null) {
    ...
    protected Renderer getRendererForSize(Effect effect, int n2, int n3) {
        return this;
    }
    ...
    public static Renderer getRenderer(FilterContext filterContext, Effect effect, int n2, int n3) {
        return Renderer.getRenderer(filterContext).getRendererForSize(effect, n2, n3);
    }
