...
public class JSWBlend_MULTIPLYPeer
extends JSWEffectPeer {
    public JSWBlend_MULTIPLYPeer(FilterContext filterContext, Renderer renderer, String string) {
    ...
    public ImageData filter(Effect effect, RenderState renderState, BaseTransform baseTransform, Rectangle rectangle, ImageData ... arrimageData) {
        ...
        for (int i2 = 0; i2 < 0 + n13; ++i2) {
            ...
            for (int i3 = 0; i3 < 0 + n12; ++i3) {
                ...
                float f31 = f23;
                float f32 = f24;
                float f33 = f25 * (f29 + 1.0f - f32) + f29 * (1.0f - f28);
                float f34 = f26 * (f30 + 1.0f - f32) + f30 * (1.0f - f28);
                float f35 = f27 * (f31 + 1.0f - f32) + f31 * (1.0f - f28);
                float f36 = f28 * (f32 + 1.0f - f32) + f32 * (1.0f - f28);
                float f37 = f33;
                float f38 = f34;
                float f39 = f35;
                float f40 = f36;
                ...
                arrn3[n15 + i3] = (int)(f37 * 255.0f) << 16 | (int)(f38 * 255.0f) << 8 | (int)(f39 * 255.0f) << 0 | (int)(f40 * 255.0f) << 24;
                f10 += f3;
                f11 += f5;
            }
            f7 += f4;
