...
public class JSWBlend_SRC_ATOPPeer
extends JSWEffectPeer {
    public JSWBlend_SRC_ATOPPeer(FilterContext filterContext, Renderer renderer, String string) {
    ...
    public ImageData filter(Effect effect, RenderState renderState, BaseTransform baseTransform, Rectangle rectangle, ImageData ... arrimageData) {
        ...
        for (int i2 = 0; i2 < 0 + n13; ++i2) {
            ...
            for (int i3 = 0; i3 < 0 + n12; ++i3) {
                ...
                float f31 = f23;
                float f32 = f24;
                float f33 = f29 * f28 + f25 * (1.0f - f32);
                float f34 = f30 * f28 + f26 * (1.0f - f32);
                float f35 = f31 * f28 + f27 * (1.0f - f32);
                float f36 = f32 * f28 + f28 * (1.0f - f32);
                float f37 = f33;
                float f38 = f34;
                float f39 = f35;
                float f40 = f36;
