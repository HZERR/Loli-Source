...
public class TypeLibUtil {
    ...
    public FindName FindName(String name, int hashVal, short maxResult) {
        Pointer p2 = Ole32.INSTANCE.CoTaskMemAlloc(((long)name.length() + 1L) * (long)Native.WCHAR_SIZE);
        WTypes.LPOLESTR olestr = new WTypes.LPOLESTR(p2);
        ...
        Ole32.INSTANCE.CoTaskMemFree(p2);
        return findName;
    }
    ...
    public static class FindName {
        ...
        public ITypeInfo[] getTInfo() {
            ...
            for (int i2 = 0; i2 < this.pcFound; ++i2) {
                values[i2] = new TypeInfo(this.pTInfo[i2]);
            }
            return values;
        }
