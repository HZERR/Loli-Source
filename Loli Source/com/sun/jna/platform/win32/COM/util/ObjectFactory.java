...
public class ObjectFactory {
    ...
    public <T> T createObject(Class<T> comInterface) {
        ...
        T t2 = this.createProxy(comInterface, d2);
        int n2 = d2.Release();
        return t2;
    }
    ...
    public void disposeAll() {
        List<WeakReference<ProxyObject>> list = this.registeredObjects;
        synchronized (list) {
            ArrayList<WeakReference<ProxyObject>> s2 = new ArrayList<WeakReference<ProxyObject>>(this.registeredObjects);
            for (WeakReference weakReference : s2) {
