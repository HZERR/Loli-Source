...
class Convert {
    ...
    public static Variant.VARIANT toVariant(Object value) {
        ...
        if (value != null) {
            for (Constructor<?> m2 : Variant.VARIANT.class.getConstructors()) {
    ...
    public static Object toJavaObject(Variant.VARIANT value, Class<?> targetClass, ObjectFactory factory, boolean addReference, boolean freeValue) {
        Object result;
        int varType;
        int n2 = varType = value != null ? value.getVarType().intValue() : 1;
        ...
        if (Byte.class.equals(targetClass) || Byte.TYPE.equals(targetClass)) {
            ...
            if (result instanceof Dispatch) {
                Dispatch d2 = (Dispatch)result;
                if (targetClass != null && targetClass.isInterface()) {
                    Object proxy = factory.createProxy(targetClass, d2);
                    if (!addReference) {
                        int n3 = d2.Release();
                    }
                    result = proxy;
    ...
    public static <T extends IComEnum> T toComEnum(Class<T> enumType, Object value) {
        try {
            IComEnum[] values;
            Method m2 = enumType.getMethod("values", new Class[0]);
            for (IComEnum t2 : values = (IComEnum[])m2.invoke(null, new Object[0])) {
                if (!value.equals(t2.getValue())) continue;
                return (T)t2;
            }
        }
