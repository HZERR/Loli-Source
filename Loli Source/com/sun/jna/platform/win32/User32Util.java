...
public final class User32Util {
    ...
    public static final WinDef.HWND createWindow(String className, String windowName, int style, int x2, int y2, int width, int height, WinDef.HWND parent, WinDef.HMENU menu, WinDef.HINSTANCE instance, WinDef.LPVOID param) {
        return User32Util.createWindowEx(0, className, windowName, style, x2, y2, width, height, parent, menu, instance, param);
    }

    public static final WinDef.HWND createWindowEx(int exStyle, String className, String windowName, int style, int x2, int y2, int width, int height, WinDef.HWND parent, WinDef.HMENU menu, WinDef.HINSTANCE instance, WinDef.LPVOID param) {
        WinDef.HWND hWnd = User32.INSTANCE.CreateWindowEx(exStyle, className, windowName, style, x2, y2, width, height, parent, menu, instance, param);
        if (hWnd == null) {
            throw new Win32Exception(Kernel32.INSTANCE.GetLastError());
        }
        return hWnd;
    ...
    public static String loadString(String location) throws UnsupportedEncodingException {
        int x2 = location.lastIndexOf(44);
        String moduleName = location.substring(0, x2);
        int index = Math.abs(Integer.parseInt(location.substring(x2 + 1)));
        ...
        x2 = User32.INSTANCE.LoadString(target, index, lpBuffer, 0);
        if (0 == x2) {
            throw new Win32Exception(Kernel32.INSTANCE.GetLastError());
        }
        if (W32APITypeMapper.DEFAULT == W32APITypeMapper.UNICODE) {
            return new String(((Pointer)lpBuffer).getPointer(0L).getCharArray(0L, x2));
        }
        return new String(((Pointer)lpBuffer).getPointer(0L).getByteArray(0L, x2), Native.getDefaultStringEncoding());
    }
