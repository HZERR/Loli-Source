...
public abstract class Rasapi32Util {
    ...
    public static WinNT.HANDLE getRasConnection(String connName) throws Ras32Exception {
        ...
        for (i2 = 0; i2 < lpcConnections.getValue(); ++i2) {
            connections[i2] = new WinRas.RASCONN();
        }
        ...
        for (i2 = 0; i2 < lpcConnections.getValue(); ++i2) {
            if (!new String(connections[i2].szEntryName).equals(connName)) continue;
            return connections[i2].hrasconn;
        }
        return null;
    }
