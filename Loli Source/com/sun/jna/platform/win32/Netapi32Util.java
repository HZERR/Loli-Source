            ...
            if (domainTrustCount.getValue() > 0) {
                ...
                for (DsGetDC.DS_DOMAIN_TRUSTS domainTrust : domainTrusts = (DsGetDC.DS_DOMAIN_TRUSTS[])domainTrustRefs.toArray(new DsGetDC.DS_DOMAIN_TRUSTS[domainTrustCount.getValue()])) {
                    ...
                    if (domainTrust.DnsDomainName != null) {
                        t2.DnsDomainName = domainTrust.DnsDomainName;
                    }
                    if (domainTrust.NetbiosDomainName != null) {
                        t2.NetbiosDomainName = domainTrust.NetbiosDomainName;
                    }
                    t2.DomainSid = domainTrust.DomainSid;
                    if (domainTrust.DomainSid != null) {
                        t2.DomainSidString = Advapi32Util.convertSidToStringSid(domainTrust.DomainSid);
                    }
                    t2.DomainGuid = domainTrust.DomainGuid;
                    if (domainTrust.DomainGuid != null) {
                        t2.DomainGuidString = Ole32Util.getStringFromGUID(domainTrust.DomainGuid);
                    }
                    t2.flags = domainTrust.Flags;
                    trusts.add(t2);
                }
            }
