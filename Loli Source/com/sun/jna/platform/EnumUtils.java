...
public class EnumUtils {
    ...
    public static <E extends Enum<E>> int toInteger(E val2) {
        Enum[] vals = (Enum[])val2.getClass().getEnumConstants();
        for (int idx = 0; idx < vals.length; ++idx) {
            if (vals[idx] != val2) continue;
            return idx;
        }
    ...
    public static <T extends FlagEnum> Set<T> setFromInteger(int flags, Class<T> clazz) {
        ...
        for (FlagEnum val2 : vals) {
            if ((flags & val2.getFlag()) == 0) continue;
            result.add(val2);
        }
        return result;
    }
