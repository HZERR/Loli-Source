...
class ELFAnalyser {
    ...
    private static BigInteger readULEB128(ByteBuffer buffer) {
        ...
        while (true) {
            byte b2 = buffer.get();
            result = result.or(BigInteger.valueOf(b2 & 0x7F).shiftLeft(shift));
            if ((b2 & 0x80) == 0) break;
            shift += 7;
        }
        return result;
    ...
    static class ELFSectionHeaders {
        ...
        public ELFSectionHeaders(boolean _64bit, boolean bigEndian, ByteBuffer headerData, RandomAccessFile raf) throws IOException {
            ...
            for (int i2 = 0; i2 < shnum; ++i2) {
                data.position(i2 * shentsize);
            ...
            for (ELFSectionHeaderEntry eshe : this.entries) {
                ...
                while (stringBuffer.position() < stringBuffer.limit() && (b2 = stringBuffer.get()) != 0) {
                    baos.write(b2);
                }
