...
abstract class GLFactory {
    ...
    private static GLGPUInfo readGPUInfo(long l2) {
        String string = GLFactory.nGetGLVendor(l2);
        String string2 = GLFactory.nGetGLRenderer(l2);
    ...
    private static boolean matches(GLGPUInfo gLGPUInfo, GLGPUInfo[] arrgLGPUInfo) {
        if (arrgLGPUInfo != null) {
            for (int i2 = 0; i2 < arrgLGPUInfo.length; ++i2) {
                if (!gLGPUInfo.matches(arrgLGPUInfo[i2])) continue;
                return true;
            }
        }
        return false;
    ...
    boolean isQualified(long l2) {
        GLGPUInfo gLGPUInfo = GLFactory.readGPUInfo(l2);
    ...
    void printDriverInformation(int n2) {
