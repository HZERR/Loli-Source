...
extends BaseShaderFactory {
    ...
    ES2ResourceFactory(Screen screen) {
        ...
        if (PrismSettings.verbose) {
            ...
            if (PlatformUtil.isEmbedded()) {
                n4 = this.context.getGLContext().getIntParam(129) * 4;
                n3 = this.context.getGLContext().getIntParam(121) * 4;
                n2 = this.context.getGLContext().getIntParam(126) * 4;
            } else {
                n4 = this.context.getGLContext().getIntParam(128);
                n3 = this.context.getGLContext().getIntParam(120);
                n2 = this.context.getGLContext().getIntParam(125);
            }
            System.out.println("Maximum number of uniform vertex components = " + n4);
            System.out.println("Maximum number of uniform fragment components = " + n3);
            System.out.println("Maximum number of varying components = " + n2);
    ...
    @Override
    public Texture createTexture(PixelFormat pixelFormat, Texture.Usage usage, Texture.WrapMode wrapMode, int n2, int n3) {
        return this.createTexture(pixelFormat, usage, wrapMode, n2, n3, false);
    }

    @Override
    public Texture createTexture(PixelFormat pixelFormat, Texture.Usage usage, Texture.WrapMode wrapMode, int n2, int n3, boolean bl) {
        return ES2Texture.create(this.context, pixelFormat, wrapMode, n2, n3, bl);
    }
    ...
    public int getRTTWidth(int n2, Texture.WrapMode wrapMode) {
        return ES2RTTexture.getCompatibleDimension(this.context, n2, wrapMode);
    }
    ...
    @Override
    public RTTexture createRTTexture(int n2, int n3, Texture.WrapMode wrapMode) {
        return this.createRTTexture(n2, n3, wrapMode, false);
    }

    @Override
    public RTTexture createRTTexture(int n2, int n3, Texture.WrapMode wrapMode, boolean bl) {
        return ES2RTTexture.create(this.context, n2, n3, wrapMode, bl);
    }
    ...
    private int computeMaxTextureSize() {
        int n2 = this.context.getGLContext().getMaxTextureSize();
        if (PrismSettings.verbose) {
            System.out.println("Maximum supported texture size: " + n2);
        }
        if (n2 > PrismSettings.maxTextureSize) {
            n2 = PrismSettings.maxTextureSize;
            if (PrismSettings.verbose) {
                System.out.println("Maximum texture size clamped to " + n2);
            }
        }
        return n2;
    }
    ...
    public Shader createShader(InputStream inputStream, Map<String, Integer> map, Map<String, Integer> map2, int n2, boolean bl, boolean bl2) {
        Map<String, Integer> map3 = this.getVertexAttributes(bl2, n2);
        String string = ES2ResourceFactory.createVertexShaderCode(bl2, n2);
        ES2Shader eS2Shader = ES2Shader.createFromSource(this.context, string, inputStream, map, map3, n2, bl);
        return eS2Shader;
    }

    private static String createVertexShaderCode(boolean bl, int n2) {
    ...
    private Map<String, Integer> getVertexAttributes(boolean bl, int n2) {
