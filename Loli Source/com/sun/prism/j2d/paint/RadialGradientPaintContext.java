...
extends MultipleGradientPaintContext {
    ...
    private void cyclicCircularGradientFillRaster(int[] arrn, int n2, int n3, int n4, int n5, int n6, int n7) {
        ...
        if (this.trivial == 0.0f) {
            int n10 = this.indexIntoGradientsArrays(0.0f);
            for (int i2 = 0; i2 < n7; ++i2) {
        ...
        for (int i4 = 0; i4 < n7; ++i4) {
            float f6 = this.a01 * (float)i4 + f2;
            float f7 = this.a11 * (float)i4 + f3;
            for (int i5 = 0; i5 < n6; ++i5) {
                ...
                float f12 = f9 + f10;
                if (f12 == 0.0f) {
                    f12 = d3 >= (double)this.focusY ? this.trivial : -this.trivial;
                }
                float f13 = (float)Math.sqrt(f11 / f12);
                arrn[n8 + i5] = this.indexIntoGradientsArrays(f13);
                f6 += this.a00;
                f7 += this.a10;
            }
            n8 += n9;
    ...
    static {
        for (int i2 = 0; i2 < sqrtLut.length; ++i2) {
            RadialGradientPaintContext.sqrtLut[i2] = (float)Math.sqrt((float)i2 / 2048.0f);
        }
    }
}
