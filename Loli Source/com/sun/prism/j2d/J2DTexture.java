...
extends BaseTexture<J2DTexResource> {
    ...
    static J2DTexture create(PixelFormat pixelFormat, Texture.WrapMode wrapMode, int n2, int n3) {
    ...
    void updateFromBuffer(BufferedImage bufferedImage, Buffer buffer, PixelFormat pixelFormat, int n2, int n3, int n4, int n5, int n6, int n7, int n8) {
        ...
        int n10 = n3 * n9 + n2 * this.setter.getNumElements();
        if (pixelGetter instanceof IntPixelGetter) {
            n8 /= 4;
        }
        int n11 = buffer.position() + n5 * n8 + n4 * pixelGetter.getNumElements();
        pixelConverter.convert(buffer, n11, n8, J2DTexture.getDstBuffer(bufferedImage), n10, n9, n6, n7);
    }
