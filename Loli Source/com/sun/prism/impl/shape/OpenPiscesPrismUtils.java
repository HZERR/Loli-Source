...
public class OpenPiscesPrismUtils {
    ...
    private static PathConsumer2D initRenderer(BasicStroke basicStroke, BaseTransform baseTransform, Rectangle rectangle, int n2, Renderer renderer) {
        int n3 = basicStroke == null && n2 == 0 ? 0 : 1;
        renderer.reset(rectangle.x, rectangle.y, rectangle.width, rectangle.height, n3);
    ...
    public static Renderer setupRenderer(Path2D path2D, BasicStroke basicStroke, BaseTransform baseTransform, Rectangle rectangle, boolean bl) {
        ...
        int n2 = path2D.getNumCommands();
        int n3 = 0;
        block7: for (int i2 = 0; i2 < n2; ++i2) {
            switch (arrby[i2]) {
                case 0: {
                    pathConsumer2D.moveTo(arrf[n3 + 0], arrf[n3 + 1]);
                    n3 += 2;
                    continue block7;
                }
                case 1: {
                    pathConsumer2D.lineTo(arrf[n3 + 0], arrf[n3 + 1]);
                    n3 += 2;
                    continue block7;
                }
                case 2: {
                    pathConsumer2D.quadTo(arrf[n3 + 0], arrf[n3 + 1], arrf[n3 + 2], arrf[n3 + 3]);
                    n3 += 4;
                    continue block7;
                }
                case 3: {
                    pathConsumer2D.curveTo(arrf[n3 + 0], arrf[n3 + 1], arrf[n3 + 2], arrf[n3 + 3], arrf[n3 + 4], arrf[n3 + 5]);
                    n3 += 6;
                    continue block7;
                }
                case 4: {
