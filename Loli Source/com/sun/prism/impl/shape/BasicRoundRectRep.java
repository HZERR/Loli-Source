...
extends BasicShapeRep {
    ...
    public static void fillRoundRect(Graphics graphics, RoundRectangle2D roundRectangle2D) {
        ...
        float f2 = roundRectangle2D.arcWidth;
        float f3 = roundRectangle2D.arcHeight;
        if (f2 > 0.0f && f3 > 0.0f) {
            graphics.fillRoundRect(roundRectangle2D.x, roundRectangle2D.y, roundRectangle2D.width, roundRectangle2D.height, f2, f3);
    ...
    public static void drawRoundRect(Graphics graphics, RoundRectangle2D roundRectangle2D) {
        ...
        float f3 = roundRectangle2D.arcHeight;
        if (f2 > 0.0f && f3 > 0.0f) {
            graphics.drawRoundRect(roundRectangle2D.x, roundRectangle2D.y, roundRectangle2D.width, roundRectangle2D.height, f2, f3);
        } else {
    ...
    private static boolean notIntEnough(float f2) {
        return (double)Math.abs(f2 - (float)Math.round(f2)) > 0.06;
    }

    private static boolean notOnIntGrid(float f2, float f3, float f4, float f5) {
        return BasicRoundRectRep.notIntEnough(f2) || BasicRoundRectRep.notIntEnough(f3) || BasicRoundRectRep.notIntEnough(f4) || BasicRoundRectRep.notIntEnough(f5);
    }
    ...
    protected static boolean isAARequiredForFill(Graphics graphics, RoundRectangle2D roundRectangle2D) {
        ...
        long l2 = baseTransform.getType();
        boolean bl2 = bl = (l2 & 0xFFFFFFFFFFFFFFF0L) != 0L;
        if (bl) {
            return true;
        }
