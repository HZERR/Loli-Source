...
implements ResourceFactory {
    ...
    public Texture getCachedTexture(Image image, Texture.WrapMode wrapMode, boolean bl) {
        ...
        if (texture2 == null) {
            ...
            long l3 = l2 = bl ? BaseResourceFactory.sizeWithMipMap(n3, n4, image.getPixelFormat()) : textureResourcePool.estimateTextureSize(n3, n4, image.getPixelFormat());
            if (!textureResourcePool.prepareForAllocation(l2)) {
                return null;
            }
            texture2 = this.createTexture(image, Texture.Usage.DEFAULT, wrapMode, bl);
            if (texture2 != null) {
                texture2.setLastImageSerial(n2);
                map.put(image, texture2);
            }
        } else if (texture2.getLastImageSerial() != n2) {
    ...
    public Texture createTexture(Image image, Texture.Usage usage, Texture.WrapMode wrapMode, boolean bl) {
        ...
        Texture texture = this.createTexture(pixelFormat, usage, wrapMode, n3 = image.getWidth(), n2 = image.getHeight(), bl);
        if (texture != null) {
            texture.update(image, 0, 0, n3, n2, true);
    ...
    @Override
    public Texture createMaskTexture(int n2, int n3, Texture.WrapMode wrapMode) {
        return this.createTexture(PixelFormat.BYTE_ALPHA, Texture.Usage.DEFAULT, wrapMode, n2, n3);
    }
