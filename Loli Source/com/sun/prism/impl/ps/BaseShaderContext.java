...
extends BaseContext {
    ...
    private static int getStockShaderIndex(MaskType maskType, Paint paint) {
        ...
        if (paint == null) {
            ...
            n2 = paint.getType().isGradient() ? ((Gradient)paint).getSpreadMethod() : 0;
        }
        return maskType.ordinal() << 4 | n3 << 2 | n2 << 0;
    }
    ...
    private Shader getPaintShader(boolean bl, MaskType maskType, Paint paint) {
        ...
        Shader shader = arrshader[n2 = BaseShaderContext.getStockShaderIndex(maskType, paint)];
        ...
        if (shader == null) {
            ...
            if (paint.getType().isGradient() && !maskType.isNewPaintStyle()) {
                ...
                int n3 = gradient.getSpreadMethod();
                if (n3 == 0) {
                    string = string + "_PAD";
                } else if (n3 == 1) {
            ...
            shader = arrshader[n2] = this.factory.createStockShader(string);
        }
        return shader;
    }

    private void updatePaintShader(BaseShaderGraphics baseShaderGraphics, Shader shader, MaskType maskType, Paint paint, float f2, float f3, float f4, float f5) {
        float f6;
        float f7;
        float f8;
        float f9;
        ...
        switch (type) {
            case LINEAR_GRADIENT: {
                PaintHelper.setLinearGradient(baseShaderGraphics, shader, (LinearGradient)paint, f9, f8, f7, f6);
                break;
            }
            case RADIAL_GRADIENT: {
                PaintHelper.setRadialGradient(baseShaderGraphics, shader, (RadialGradient)paint, f9, f8, f7, f6);
                break;
            }
            case IMAGE_PATTERN: {
                PaintHelper.setImagePattern(baseShaderGraphics, shader, (ImagePattern)paint, f9, f8, f7, f6);
            }
        }
    }
    ...
    Shader validatePaintOp(BaseShaderGraphics baseShaderGraphics, BaseTransform baseTransform, MaskType maskType, float f2, float f3, float f4, float f5) {
        return this.validatePaintOp(baseShaderGraphics, baseTransform, maskType, null, f2, f3, f4, f5);
    }

    Shader validatePaintOp(BaseShaderGraphics baseShaderGraphics, BaseTransform baseTransform, MaskType maskType, float f2, float f3, float f4, float f5, float f6, float f7, float f8, float f9, float f10, float f11) {
    ...
    Shader validatePaintOp(BaseShaderGraphics baseShaderGraphics, BaseTransform baseTransform, MaskType maskType, Texture texture, float f2, float f3, float f4, float f5, float f6, float f7, float f8, float f9, float f10, float f11) {
        if (this.state.lastConst1 != f6 || this.state.lastConst2 != f7 || this.state.lastConst3 != f8 || this.state.lastConst4 != f9 || this.state.lastConst5 != f10 || this.state.lastConst6 != f11) {
            this.flushVertexBuffer();
            this.state.lastConst1 = f6;
            this.state.lastConst2 = f7;
        ...
        return this.validatePaintOp(baseShaderGraphics, baseTransform, maskType, texture, f2, f3, f4, f5);
    }
    ...
    Shader validatePaintOp(BaseShaderGraphics baseShaderGraphics, BaseTransform baseTransform, Texture texture, float f2, float f3, float f4, float f5) {
        return this.validatePaintOp(baseShaderGraphics, baseTransform, MaskType.TEXTURE, texture, f2, f3, f4, f5);
    }

    Shader validatePaintOp(BaseShaderGraphics baseShaderGraphics, BaseTransform baseTransform, MaskType maskType, Texture texture, float f2, float f3, float f4, float f5) {
        if (maskType == null) {
        ...
        if (this.externalShader == null) {
            ...
            this.updatePaintShader(baseShaderGraphics, (Shader)object, maskType, paint, f2, f3, f4, f5);
    ...
    Shader validateTextureOp(BaseShaderGraphics baseShaderGraphics, BaseTransform baseTransform, Texture[] arrtexture, PixelFormat pixelFormat) {
        ...
        if (null != shader) {
            this.checkState(baseShaderGraphics, 15, baseTransform, shader);
            int n2 = Math.max(0, Math.min(arrtexture.length, 4));
            for (int i2 = 0; i2 < n2; ++i2) {
                this.setTexture(i2, arrtexture[i2]);
            }
            this.updatePerVertexColor(null, baseShaderGraphics.getExtraAlpha());
        }
        return shader;
    ...
    private void checkState(BaseShaderGraphics baseShaderGraphics, int n2, BaseTransform baseTransform, Shader shader) {
        Object object;
        ...
        if (!((n2 & 2) == 0 || this.state.isXformValid && baseTransform.equals(this.state.lastTransform))) {
            this.flushVertexBuffer();
    ...
    private void setTexture(int n2, Texture texture) {
        ...
        if (texture != this.state.lastTextures[n2]) {
            ...
            ((State)this.state).lastTextures[n2] = texture;
        }
    }
        ...
        if (this.state == null || bl2 != this.state.lastState3D || renderTarget != this.state.lastRenderTarget || nGCamera != this.state.lastCamera || bl != this.state.lastDepthTest) {
            ...
            if (bl2 != this.state.lastState3D) {
                ...
                for (int i2 = 0; i2 != this.state.lastTextures.length; ++i2) {
                    ((State)this.state).lastTextures[i2] = null;
                }
                if (bl2) {
