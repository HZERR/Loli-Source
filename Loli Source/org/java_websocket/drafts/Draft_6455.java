...
extends Draft {
    ...
    public HandshakeState acceptHandshakeAsServer(ClientHandshake handshakedata) throws InvalidHandshakeException {
        ...
        int v2 = this.readVersion(handshakedata);
        if (v2 != 13) {
    ...
    private HandshakeState containsRequestedProtocol(String requestedProtocol) {
        for (IProtocol knownProtocol : this.knownProtocols) {
            ...
            log.trace("acceptHandshake - Matching protocol found: {}", (Object)this.protocol);
            return HandshakeState.MATCHED;
        }
        return HandshakeState.NOT_MATCHED;
    }
    ...
    public ByteBuffer createBinaryFrame(Framedata framedata) {
        ...
        if (log.isTraceEnabled()) {
            log.trace("afterEnconding({}): {}", (Object)framedata.getPayloadData().remaining(), (Object)(framedata.getPayloadData().remaining() > 1000 ? "too big to display" : new String(framedata.getPayloadData().array())));
        }
    ...
    private Framedata translateSingleFrame(ByteBuffer buffer) throws IncompleteException, InvalidDataException {
        ...
        if (log.isTraceEnabled()) {
            log.trace("afterDecoding({}): {}", (Object)frame.getPayloadData().remaining(), (Object)(frame.getPayloadData().remaining() > 1000 ? "too big to display" : new String(frame.getPayloadData().array())));
        }
        frame.isValid();
        return frame;
    }
    ...
    private void translateSingleFrameCheckLengthLimit(long length) throws LimitExceededException {
        ...
        if (length > (long)this.maxFrameSize) {
            log.trace("Payload limit reached. Allowed: {} Current: {}", (Object)this.maxFrameSize, (Object)length);
    ...
    public List<Framedata> translateFrame(ByteBuffer buffer) throws InvalidDataException {
        ...
        while (true) {
            ...
            try {
                ...
                ByteBuffer extendedframe = ByteBuffer.allocate(this.checkAlloc(e2.getPreferredSize()));
    ...
    private byte[] toByteArray(long val2, int bytecount) {
        ...
        for (int i2 = 0; i2 < bytecount; ++i2) {
            buffer[i2] = (byte)(val2 >>> highest - 8 * i2);
        }
        return buffer;
    }
    ...
    private void processFrameBinary(WebSocketImpl webSocketImpl, Framedata frame) {
        try {
            ...
            this.logRuntimeException(webSocketImpl, e2);
        }
    }

    private void logRuntimeException(WebSocketImpl webSocketImpl, RuntimeException e2) {
        log.error("Runtime exception during onWebsocketMessage", e2);
    ...
    private void processFrameIsFin(WebSocketImpl webSocketImpl, Framedata frame) throws InvalidDataException {
        ...
        if (this.currentContinuousFrame.getOpcode() == Opcode.TEXT) {
            ...
            try {
                ...
                this.logRuntimeException(webSocketImpl, e3);
            }
        }
    ...
    private void checkBufferLimit() throws LimitExceededException {
        ...
        if (totalSize > (long)this.maxFrameSize) {
            this.clearBufferList();
            log.trace("Payload limit reached. Allowed: {} Current: {}", (Object)this.maxFrameSize, (Object)totalSize);
