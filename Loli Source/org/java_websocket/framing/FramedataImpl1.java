...
implements Framedata {
    ...
    public void append(Framedata nextframe) {
        ByteBuffer b2 = nextframe.getPayloadData();
        if (this.unmaskedpayload == null) {
            this.unmaskedpayload = ByteBuffer.allocate(b2.remaining());
            b2.mark();
            this.unmaskedpayload.put(b2);
            b2.reset();
        } else {
            b2.mark();
            this.unmaskedpayload.position(this.unmaskedpayload.limit());
            this.unmaskedpayload.limit(this.unmaskedpayload.capacity());
            if (b2.remaining() > this.unmaskedpayload.remaining()) {
                ByteBuffer tmp = ByteBuffer.allocate(b2.remaining() + this.unmaskedpayload.capacity());
    ...
    public boolean equals(Object o2) {
        ...
        FramedataImpl1 that = (FramedataImpl1)o2;
