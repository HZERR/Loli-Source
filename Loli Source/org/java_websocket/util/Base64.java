...
public class Base64 {
    ...
    public static byte[] encodeBytesToBytes(byte[] source, int off, int len, int options) throws IOException {
        ...
        int lineLength = 0;
        while (d2 < len2) {
            Base64.encode3to4(source, d2 + off, 3, outBuff, e3, options);
        ...
        if (d2 < len) {
            Base64.encode3to4(source, d2 + off, len - d2, outBuff, e3, options);
            e3 += 4;
        }
        if (e3 <= outBuff.length - 1) {
            byte[] finalOut = new byte[e3];
            System.arraycopy(outBuff, 0, finalOut, 0, e3);
            return finalOut;
        }
        return outBuff;
    }
