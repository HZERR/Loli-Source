...
implements WebSocket {
    ...
    public void decode(ByteBuffer socketBuffer) {
        ...
        log.trace("process({}): ({})", (Object)socketBuffer.remaining(), (Object)(socketBuffer.remaining() > 1000 ? "too big to display" : new String(socketBuffer.array(), socketBuffer.position(), socketBuffer.remaining())));
                ...
                try {
                            ...
                            for (Draft d2 : this.knownDrafts) {
                                ...
                                try {
                                    ...
                                    try {
                                        ...
                                        log.trace("Closing due to wrong handshake. Possible handshake rejection", e2);
                                        this.closeConnectionDueToWrongHandshake(e2);
                                        return false;
                                    }
                                    catch (RuntimeException e3) {
                                        log.error("Closing due to internal server error", e3);
                                        this.wsl.onWebsocketError(this, e3);
                                        this.closeConnectionDueToInternalServerError(e3);
                                        return false;
                                    }
                                    this.write(d2.createHandshake(d2.postProcessHandshakeResponseAsServer(handshake, response)));
                                    this.draft = d2;
                                    this.open(handshake);
                                    return true;
                                }
                    ...
                    if (handshakestate == HandshakeState.MATCHED) {
                        try {
                            ...
                            log.trace("Closing due to invalid data exception. Possible handshake rejection", e4);
                            this.flushAndClose(e4.getCloseCode(), e4.getMessage(), false);
                            return false;
                        }
                        catch (RuntimeException e5) {
                            log.error("Closing since client was never connected", e5);
                    ...
                    log.trace("Closing due to invalid handshake", e6);
                    this.close(e6);
                }
            }
    ...
    private void decodeFrames(ByteBuffer socketBuffer) {
        try {
            ...
            for (Framedata f2 : frames) {
                log.trace("matched frame: {}", (Object)f2);
                this.draft.processFrame(this, f2);
            }
        }
        catch (LimitExceededException e2) {
            if (e2.getLimit() == Integer.MAX_VALUE) {
                log.error("Closing due to invalid size of frame", e2);
                this.wsl.onWebsocketError(this, e2);
            }
            this.close(e2);
        }
        catch (InvalidDataException e3) {
            log.error("Closing due to invalid data in frame", e3);
            this.wsl.onWebsocketError(this, e3);
            this.close(e3);
        }
    }
            ...
            if (this.readyState == ReadyState.OPEN) {
                ...
                if (this.draft.getCloseHandshakeType() != CloseHandshakeType.NONE) {
                    try {
                        ...
                        log.error("generated frame is invalid", e3);
                        this.wsl.onWebsocketError(this, e3);
        ...
        if (this.channel != null) {
            try {
                ...
                if (e2.getMessage().equals("Broken pipe")) {
                    log.trace("Caught IOException: Broken pipe during closeConnection()", e2);
                }
                log.error("Exception during channel.close()", e2);
                this.wsl.onWebsocketError(this, e2);
            }
        }
        try {
    ...
    public synchronized void flushAndClose(int code, String message, boolean remote) {
        ...
        try {
            ...
            log.error("Exception in onWebsocketClosing", e2);
            this.wsl.onWebsocketError(this, e2);
        }
        if (this.draft != null) {
            this.draft.reset();
    ...
    private void send(Collection<Framedata> frames) {
        ...
        for (Framedata f2 : frames) {
            log.trace("send frame: {}", (Object)f2);
    ...
    public void startHandshake(ClientHandshakeBuilder handshakedata) throws InvalidHandshakeException {
        ...
        try {
            ...
            log.error("Exception in startHandshake", e3);
            this.wsl.onWebsocketError(this, e3);
    ...
    private void write(ByteBuffer buf) {
        log.trace("write({}): {}", (Object)buf.remaining(), (Object)(buf.remaining() > 1000 ? "too big to display" : new String(buf.array())));
