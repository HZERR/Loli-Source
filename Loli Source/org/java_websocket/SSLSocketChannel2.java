...
WrappedByteChannel {
    ...
    private void consumeFutureUninterruptible(Future<?> f2) {
        try {
            while (true) {
                try {
                    f2.get();
    ...
    private synchronized void processHandshake() throws IOException {
        ...
        if (!this.tasks.isEmpty()) {
            ...
            while (it.hasNext()) {
                ...
                if (this.isBlocking()) {
                    this.consumeFutureUninterruptible(f2);
                }
                return;
            }
        }
    ...
    private synchronized ByteBuffer wrap(ByteBuffer b2) throws SSLException {
        this.outCrypt.compact();
        this.writeEngineResult = this.sslEngine.wrap(b2, this.outCrypt);
        this.outCrypt.flip();
    ...
    public SelectableChannel configureBlocking(boolean b2) throws IOException {
        return this.socketChannel.configureBlocking(b2);
    }
