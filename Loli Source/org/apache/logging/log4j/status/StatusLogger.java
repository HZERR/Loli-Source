...
extends AbstractLogger {
    ...
    public void removeListener(StatusListener listener) {
        ...
        try {
            ...
            for (StatusListener l2 : this.listeners) {
                int level = l2.getStatusLevel().intLevel();
     ...
     */
    @Override
    public void log(Marker marker, String fqcn, Level level, Message msg, Throwable t2) {
        StackTraceElement element = null;
        if (fqcn != null) {
            element = this.getStackTraceElement(fqcn, Thread.currentThread().getStackTrace());
        }
        StatusData data = new StatusData(element, level, msg, t2);
        this.msgLock.lock();
        try {
        ...
        if (this.listeners.size() > 0) {
            ...
            this.logger.log(marker, fqcn, level, msg, t2);
        }
    }
    ...
    @Override
    protected boolean isEnabled(Level level, Marker marker, String data, Throwable t2) {
        return this.isEnabled(level, marker);
    }
    ...
    @Override
    protected boolean isEnabled(Level level, Marker marker, Object data, Throwable t2) {
        return this.isEnabled(level, marker);
    }

    @Override
    protected boolean isEnabled(Level level, Marker marker, Message data, Throwable t2) {
        return this.isEnabled(level, marker);
    }
