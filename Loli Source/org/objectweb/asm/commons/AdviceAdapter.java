...
implements Opcodes {
    ...
    public void visitInsn(int opcode) {
        if (this.constructor) {
            switch (opcode) {
                ...
                case 90: {
                    int s2 = this.stackFrame.size();
                    this.stackFrame.add(s2 - 2, this.stackFrame.get(s2 - 1));
                    break;
                }
                case 91: {
                    int s2 = this.stackFrame.size();
                    this.stackFrame.add(s2 - 3, this.stackFrame.get(s2 - 1));
                    break;
                }
                case 92: {
                    int s2 = this.stackFrame.size();
                ...
                case 94: {
                    int s2 = this.stackFrame.size();
                    this.stackFrame.add(s2 - 4, this.stackFrame.get(s2 - 1));
                    this.stackFrame.add(s2 - 4, this.stackFrame.get(s2 - 1));
                    break;
                }
                case 95: {
    ...
    public void visitVarInsn(int opcode, int var2) {
        super.visitVarInsn(opcode, var2);
        if (this.constructor) {
            switch (opcode) {
                case 21: 
                case 23: {
    ...
    public void visitFieldInsn(int opcode, String owner, String name, String desc) {
        ...
        if (this.constructor) {
            ...
            boolean longOrDouble = c2 == 'J' || c2 == 'D';
            switch (opcode) {
                case 178: {
                    this.pushValue(OTHER);
    ...
    public void visitMethodInsn(int opcode, String owner, String name, String desc) {
        ...
        if (this.constructor) {
            ...
            for (int i2 = 0; i2 < types.length; ++i2) {
                this.popValue();
                if (types[i2].getSize() != 2) continue;
                this.popValue();
            }
            switch (opcode) {
                case 182: 
    ...
    private void pushValue(Object o2) {
        this.stackFrame.add(o2);
    }
