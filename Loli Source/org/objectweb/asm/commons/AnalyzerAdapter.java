...
extends MethodAdapter {
    ...
    public AnalyzerAdapter(String owner, int access, String name, String desc, MethodVisitor mv) {
        ...
        block7: for (int i2 = 0; i2 < types.length; ++i2) {
            ...
            switch (type.getSort()) {
                ...
                case 9: {
                    this.locals.add(types[i2].getDescriptor());
                    continue block7;
                }
                default: {
                    this.locals.add(types[i2].getInternalName());
                }
            }
        }
    }
    ...
    private static void visitFrameTypes(int n2, Object[] types, List<Object> result) {
    ...
    public void visitTypeInsn(int opcode, String type) {
        if (opcode == 187) {
            ...
            for (int i2 = 0; i2 < this.labels.size(); ++i2) {
                this.uninitializedTypes.put(this.labels.get(i2), type);
            }
        }
        if (this.mv != null) {
    ...
    public void visitMethodInsn(int opcode, String owner, String name, String desc) {
        ...
        if (opcode != 184) {
            ...
            if (opcode == 183 && name.charAt(0) == '<') {
                int i2;
                Object u2 = t2 == Opcodes.UNINITIALIZED_THIS ? this.owner : this.uninitializedTypes.get(t2);
                for (i2 = 0; i2 < this.locals.size(); ++i2) {
                    if (this.locals.get(i2) != t2) continue;
                    this.locals.set(i2, u2);
                }
                for (i2 = 0; i2 < this.stack.size(); ++i2) {
                    if (this.stack.get(i2) != t2) continue;
                    this.stack.set(i2, u2);
                }
            }
        }
        this.pushDesc(desc);
    ...
    private Object get(int local) {
        this.maxLocals = Math.max(this.maxLocals, local);
