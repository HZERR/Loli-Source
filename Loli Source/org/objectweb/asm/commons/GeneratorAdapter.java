...
extends LocalVariablesSorter {
    ...
    public void pushMethodType(MethodType methodType) {
        this.mv.visitLdcInsn(methodType);
    }

    public void pushMethodType(MethodHandle methodHandle) {
        this.mv.visitLdcInsn(methodHandle);
    }
    ...
    public void loadArgs(int arg, int count) {
        ...
        for (int i2 = 0; i2 < count; ++i2) {
            Type t2 = this.argumentTypes[arg + i2];
            this.loadInsn(t2, index);
            index += t2.getSize();
        }
    }
        ...
        if (keys.length > 0) {
            ...
            if (useTable) {
                ...
                for (i3 = 0; i3 < range; ++i3) {
                    ...
                    generator.generateCase(i3 + min, end);
                }
            } else {
                int i4;
                ...
                for (i4 = 0; i4 < len; ++i4) {
                    this.mark(labels[i4]);
                    generator.generateCase(keys[i4], end);
                }
            }
        }
        this.mark(def);
