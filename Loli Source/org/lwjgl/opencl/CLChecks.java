...
final class CLChecks {
    ...
    static int calculateBufferRectSize(PointerBuffer offset, PointerBuffer region, long row_pitch, long slice_pitch) {
        ...
        long z = offset.get(2);
        if (LWJGLUtil.DEBUG && (x2 < 0L || y2 < 0L || z < 0L)) {
            throw new IllegalArgumentException("Invalid cl_mem host offset: " + x2 + ", " + y2 + ", " + z);
        }
        ...
        return (int)(z * slice_pitch + y2 * row_pitch + x2 + w2 * h2 * d2);
    }
