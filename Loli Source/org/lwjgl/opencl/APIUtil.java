...
final class APIUtil {
    ...
    static int getSize(PointerBuffer lengths) {
        long size = 0L;
        for (int i2 = lengths.position(); i2 < lengths.limit(); ++i2) {
            size += lengths.get(i2);
        }
        return (int)size;
    }
    ...
    static CLPlatform getCLPlatform(PointerBuffer properties) {
        ...
        for (int k2 = 0; k2 < keys && (key = properties.get(k2 << 1)) != 0L; ++k2) {
            if (key != 4228L) continue;
            platformID = properties.get((k2 << 1) + 1);
            break;
        }
        if (platformID == 0L) {
    ...
    static ByteBuffer getNativeKernelArgs(long user_func_ref, CLMem[] clMems, long[] sizes) {
        ...
        if (clMems == null) {
            ...
            int byteIndex = 12;
            for (int i2 = 0; i2 < clMems.length; ++i2) {
                if (LWJGLUtil.DEBUG && !clMems[i2].isValid()) {
                    throw new IllegalArgumentException("An invalid CLMem object was specified.");
                }
                args.putInt(byteIndex, (int)sizes[i2]);
