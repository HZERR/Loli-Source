...
public class Cursor {
            ...
            case 3: {
                ...
                for (int i3 = 0; i3 < numImages; ++i3) {
                    ...
                    for (int j2 = 0; j2 < size; ++j2) {
                        int index = j2 + i3 * size;
                        int alpha = images_copy.get(index) >> 24 & 0xFF;
                        if (alpha == 255) continue;
                        images_copy.put(index, 0);
                    }
                    Object handle = Mouse.getImplementation().createCursor(width, height, xHotspot, yHotspot, 1, images_copy, null);
                    long delay = delays != null ? (long)delays.get(i3) : 0L;
                    long timeout = System.currentTimeMillis();
                    cursors[i3] = new CursorElement(handle, delay, timeout);
                    images_copy.position(width * height * (i3 + 1));
                }
                break;
            }
            case 1: {
    ...
    private static void convertARGBtoABGR(IntBuffer imageBuffer) {
        for (int i2 = 0; i2 < imageBuffer.limit(); ++i2) {
            int argbColor = imageBuffer.get(i2);
