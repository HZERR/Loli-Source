...
public class MappedObjectTransformer {
    ...
    static byte[] transformMappedAPI(String className, byte[] bytecode) {
        ...
        ClassAdapter cv = ta = new TransformationAdapter(cw, className);
        if (className_to_subtype.containsKey(className)) {
            cv = MappedObjectTransformer.getMethodGenAdapter(className, cv);
        }
        ...
        switch (methodInsn.getOpcode()) {
            case 182: {
                ...
                if ("dup".equals(methodInsn.name) && methodInsn.desc.equals("()L" + MAPPED_OBJECT_JVM + ";")) {
                    i2 = MappedObjectTransformer.replace(instructions, i2, methodInsn, MappedObjectTransformer.generateDupInstructions(methodInsn));
                    break;
                }
                if ("slice".equals(methodInsn.name) && methodInsn.desc.equals("()L" + MAPPED_OBJECT_JVM + ";")) {
                    i2 = MappedObjectTransformer.replace(instructions, i2, methodInsn, MappedObjectTransformer.generateSliceInstructions(methodInsn));
                    break;
                }
                if ("runViewConstructor".equals(methodInsn.name) && "()V".equals(methodInsn.desc)) {
                    i2 = MappedObjectTransformer.replace(instructions, i2, methodInsn, MappedObjectTransformer.generateRunViewConstructorInstructions(methodInsn));
                    break;
                }
                if ("copyTo".equals(methodInsn.name) && methodInsn.desc.equals("(L" + MAPPED_OBJECT_JVM + ";)V")) {
                    i2 = MappedObjectTransformer.replace(instructions, i2, methodInsn, MappedObjectTransformer.generateCopyToInstructions(mappedType));
                    break;
                }
                if (!"copyRange".equals(methodInsn.name) || !methodInsn.desc.equals("(L" + MAPPED_OBJECT_JVM + ";I)V")) break;
                i2 = MappedObjectTransformer.replace(instructions, i2, methodInsn, MappedObjectTransformer.generateCopyRangeInstructions(mappedType));
                break;
            }
            case 183: {
            ...
            case 184: {
                ...
                i2 = MappedObjectTransformer.replace(instructions, i2, methodInsn, MappedObjectTransformer.generateMapInstructions(mappedType, methodInsn.owner, isMapDirectMethod, isMallocMethod));
            }
        }
        return i2;
    }
    ...
    static int transformArrayAccess(InsnList instructions, int i, Map<AbstractInsnNode, Frame<BasicValue>> frameMap, VarInsnNode loadInsn, MappedSubtypeInfo mappedSubtype, int var) {
        block8: {
            loadStackSize = frameMap.get(loadInsn).getStackSize() + 1;
            nextInsn = loadInsn;
            do lbl-1000:
            // 3 sources

            {
                ...
                loadInsn.var = var;
                instructions.insert((AbstractInsnNode)loadInsn, new MethodInsnNode(182, mappedSubtype.className, "capacity", "()I"));
                return i + 1;
        ...
        public MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
            ...
            return new MethodNode(access, name, desc, signature, exceptions){
                ...
                private Frame<BasicValue>[] analyse() throws AnalyzerException {
                    Analyzer<BasicValue> a2 = new Analyzer<BasicValue>(new SimpleVerifier());
                    a2.analyze(TransformationAdapter.this.className, this);
                    return a2.getFrames();
                }
