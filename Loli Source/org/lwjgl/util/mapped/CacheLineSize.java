...
final class CacheLineSize {
    ...
    static int getCacheLineSize() {
        ...
        try {
            ...
            boolean found = false;
            for (int i3 = MAX_SIZE; i3 >= 1; i3 >>= 1) {
                long avgTime;
                long time = CacheLineSize.doTest(2, 100000, i3, memory, completionService);
                if (totalTime > 0L && (double)time / (double)(avgTime = totalTime / (long)count) > TIME_THRESHOLD) {
                    cacheLineSize = (i3 << 1) * 4;
                    found = true;
                    break;
                }
                totalTime += time;
    ...
    private static long doTest(int threads, int repeats, int padding, IntBuffer memory, ExecutorCompletionService<Long> completionService) {
        for (int i2 = 0; i2 < threads; ++i2) {
            CacheLineSize.submitTest(completionService, i2, repeats, memory, padding);
        }
